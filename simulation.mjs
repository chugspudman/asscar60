...
const STRANGE_EFFECT_SUMMARIES = [
  ...
  ["religious speedpiphany!", "Control blessing this stint"],
  ...
  [
    "gets SPEED MADNESS!",
    "Control, Overtaking, and Stamina maxed this stint; must recover next race; Given The Mark of The Speed God permanently",
  ],
...
function finishTieBreaker(a, b) {
  const aPrevious = Math.max(
    0,
    ...((a.positionRecords || [])
      .filter((record) => record.time < (a.finishTime ?? Infinity))
      .map((record) => record.position)),
  );
  const bPrevious = Math.max(
    0,
    ...((b.positionRecords || [])
      .filter((record) => record.time < (b.finishTime ?? Infinity))
      .map((record) => record.position)),
  );
  return bPrevious - aPrevious
    || (b.startingGridPosition ?? 0) - (a.startingGridPosition ?? 0)
...
function snapshotLivePositions(entries, timestamp) {
  const states = [...entries].sort((a, b) => (
  ...
  return states.map((entry, index) => {
    const completedLaps = Math.min(TOTAL_LAPS, Math.floor(entry.position));
    const progress = completedLaps >= TOTAL_LAPS ? 0 : clamp(entry.position - completedLaps, 0, 0.999999);
    const driver = entry.dnfDriver || driverForLap(entry, Math.min(TOTAL_LAPS, completedLaps + 1));
    return {
      id: entry.id,
      position: index + 1,
      completedLaps,
      progress,
      raceProgress: entry.position,
      elapsed: entry.status === "finished"
        ? entry.finishTime
        : entry.status === "dnf"
          ? entry.dnfTime
          : timestamp,
      ...
      lastLapTime: entry.lastLapTime,
      gap: index === 0 ? 0 : Math.max(0, leader.position - entry.position) * 50,
      driver: driver.name,
      driverPronouns: driver.pronouns,
      status: entry.status,
      timestamp,
    };
  });
...
function snapshotAtTime(entries, timestamp, orderHistory = null) {
  if (entries.some((entry) => entry.positionRecords?.length)) {
    const states = entries.map((entry) => {
      ...
      const position = Math.max(rawPosition, completed.length);
      ...
      const progress = completedLaps >= TOTAL_LAPS ? 0 : clamp(position - completedLaps, 0, 0.999999);
    ...
    states.sort((a, b) => (
      ...
      || (a.status === "finished" && b.status === "finished"
        ? (a.finishTime ?? Infinity) - (b.finishTime ?? Infinity)
          || finishTieBreaker(a.entry, b.entry)
        : 0)
      || b.position - a.position
...
function perfectRaceMinutes(courseLength) {
  return 10 * clamp(courseLength, 1, 6);
}

function performanceTimeMultiplier(_pace, speed) {
  return linearScale(speed, 1.054, 1);
}
...
function addActionEvents(events, entries, duration, random) {
  const chronological = [...events].sort(compareRaceEvents);
...
function compareRaceEvents(a, b) {
  return a.time - b.time
    || (a.finishOrderIndex ?? Number.MAX_SAFE_INTEGER)
      - (b.finishOrderIndex ?? Number.MAX_SAFE_INTEGER);
}

function throttleFeedEvents(events, minimumSpacing = 2) {
  const sorted = [...events].sort(compareRaceEvents);
...
const ALWAYS_STRANGE_EFFECTS = [
  ...
  { tone: "good", target: "driver", stat: "control", delta: 1, message: (_, driver) => `${driver.name} hears the voice of The Speed God and has a religious speedpiphany!` },
  ...
  { tone: "bad", target: "driver", stat: "control", delta: -1, message: (_, driver) => `${driver.name} is tempted by the mutterings of The Decelerator!` },
...
function lapCandidate(entry, lap, course, condition, random) {
  ...
  const effectivePace = clamp(driver.pace + (bonuses.pace || 0) - driverFatigue, 1, 10);
  const candidate = {
    entry,
    driver,
    lap,
...
export function simulateRace(entries, seed = "opening-bell", options = {}) {
  ...
  const scheduleStrange = (entry, part) => {
    ...
    const markBonus = driver.speed_mark ? 2 : 0;
    const strangeChance = Math.min(
      1,
  ...
  const raceSpeed = (stats) => 1 / basePartDuration(course.length, 10, stats.speed);
  ...
  while (racers.some((entry) => entry.status === "running") && time < 7200) {
    ...
    for (const entry of runningAtTick) {
      ...
      const rawEndPosition = entry.position + speed * remainingSecond;
      entry.position = clamp(rawEndPosition, 0, TOTAL_LAPS);
    ...
    const overtakeEventTime = (entry) => {
      const state = tickStates.get(entry.id);
      const checkpoint = entry.nextCheckpoint;
      if (
        state
        && state.startPosition < checkpoint
        && state.endPosition >= checkpoint
        && state.endPosition > state.startPosition
      ) {
        const crossingTime = time
          + (checkpoint - state.startPosition) / (state.endPosition - state.startPosition);
        return Math.max(time + 0.001, crossingTime - 0.01);
      }
      return nextTime;
    };
    ...
    if (nextTime > 6) {
      ...
      const physicalGapAhead = (attacker, defender) => {
        const attackerProgress = attacker.position - Math.floor(attacker.position);
        const defenderProgress = defender.position - Math.floor(defender.position);
        const gap = (defenderProgress - attackerProgress + 1) % 1;
        return gap <= 0.000001 ? 1 : gap;
      };
      const physicalPositionAhead = (attacker, defender) => {
        const gap = physicalGapAhead(attacker, defender);
        return attacker.position + gap;
      };
      const physicalTargetAhead = (attacker, attackerSpeed) => runningAtTick
        .filter((defender) => (
          defender.id !== attacker.id
          && defender.status === "running"
          && !involved.has(defender.id)
        ))
        .map((defender) => ({
          defender,
          gap: physicalGapAhead(attacker, defender),
        }))
        .filter(({ gap }) => gap > 0 && gap <= attackerSpeed)
        .sort((a, b) => a.gap - b.gap)[0]?.defender || null;

      const attackers = runningAtTick
        .filter((entry) => entry.status === "running")
        .sort((a, b) => b.position - a.position);
      for (const attacker of attackers) {
        if (involved.has(attacker.id)) continue;
        if (time < attacker.overtakeCooldownUntil) continue;
        const attackingStats = effectiveStats(attacker);
        const attackerSpeed = raceSpeed(attackingStats);
        const defender = physicalTargetAhead(attacker, attackerSpeed);
        if (!defender) continue;
        const defenderStats = effectiveStats(defender);
        const defenderSpeed = raceSpeed(defenderStats);
        const pair = `${attacker.id}:${defender.id}`;
        if (attacker.tickOvertakePairs.has(pair)) continue;
        attacker.tickOvertakePairs.add(pair);
        involved.add(attacker.id);
        involved.add(defender.id);
        const eventTime = overtakeEventTime(attacker);
        const attackerEventPosition = () => Math.max(
          attacker.position,
          attacker.completedLaps + 0.0001,
        );
        const defenderEventPosition = () => Math.max(
          defender.position,
          defender.completedLaps + 0.0001,
        );
        if (attackerWinsOvertake(
          attackingStats.overtaking,
          defenderStats.overtaking,
          random,
        )) {
          const passPosition = physicalPositionAhead(attacker, defender);
          recordPosition(defender, eventTime);
          attacker.position = Math.min(TOTAL_LAPS, passPosition + defenderSpeed * 2 + 0.001);
          tickStates.get(attacker.id).endPosition = Math.max(
            tickStates.get(attacker.id).endPosition,
            attacker.position,
          );
          recordPosition(attacker, eventTime, attacker.position);
          events.push({
            time: eventTime,
            type: "overtake",
            entryId: attacker.id,
            relatedEntryId: defender.id,
            ...positionMeta(attackerEventPosition()),
            standings: snapshotLivePositions(racers, eventTime),
            message: [
              `${attackingStats.driver.name} just manages to slip past ${defenderStats.driver.name} in the straight.`,
              `${attackingStats.driver.name} sees an opening and blows past ${defenderStats.driver.name}.`,
              `${attackingStats.driver.name} leaves ${defenderStats.driver.name} in the dust.`,
              `${attackingStats.driver.name} overtakes ${defenderStats.driver.name} in the straight.`,
            ][Math.floor(random() * 4)],
            category: "event",
          });
          ...
          if (random() < 1 / 20) {
            ...
            events.push({
              ...
              type: "incident",
              entryId: defender.id,
              ...positionMeta(defenderEventPosition()),
              standings: snapshotLivePositions(racers, eventTime + 0.01),
              message: `${defenderStats.driver.name} attempted to deny the overtake but suffered a mishap, losing 3 seconds.`,
              category: "event",
            });
          }
        } else {
          ...
          events.push({
            ...
            standings: snapshotLivePositions(racers, eventTime),
            message: [
              `${defenderStats.driver.name} sees ${attackingStats.driver.name} in the mirrors and is able to fend off the overtake.`,
              `${defenderStats.driver.name} shuts down an overtake attempt by ${attackingStats.driver.name}.`,
              `${attackingStats.driver.name} goes for the overtake against ${defenderStats.driver.name} but isn't able to pull it off.`,
            ][Math.floor(random() * 3)],
            category: "event",
          });
          if (hasCoachSpecialty(attacker, "specialty-9")) {
    ...
    for (const entry of runningAtTick) {
      ...
      let tickEndPosition = tickState.endPosition;

      if (entry.pendingStrange && startPosition < entry.pendingStrange.position && tickEndPosition >= entry.pendingStrange.position) {
        const effect = entry.pendingStrange;
        const eventTime = nextTime - (tickEndPosition - effect.position) / Math.max(speed, 0.0001);
      ...
      const endSegment = Math.min(totalSegments - 1, Math.floor(tickEndPosition * segmentCount));
      for (let segmentNumber = startSegment; segmentNumber <= endSegment; segmentNumber += 1) {
        ...
        if (segmentPosition < startPosition || segmentPosition > tickEndPosition) continue;
      ...
      for (const segmentNumber of crossedSegments) {
        ...
        const segmentPosition = (segmentNumber + 0.5) / segmentCount;
        const eventTime = nextTime - (tickEndPosition - segmentPosition) / Math.max(speed, 0.0001);
      ...
      while (entry.status === "running" && tickEndPosition >= entry.nextCheckpoint) {
        ...
        entry.position = Math.max(entry.position, checkpoint);
        ...
        if (checkpoint === TOTAL_LAPS) {
          ...
          events.push({
            ...
            message: isFirstFinisher
              ...
              : `${driver.name} in ${entry.carName} crosses the finish line with a time of ${formatRaceTime(crossingTime)}.`,
          });
          break;
        }
  ...
  const officialFinishOrder = rankEntries(racers).filter((entry) => entry.status === "finished");
  const finishEventsByEntry = new Map(events
    .filter((event) => event.type === "winner" || event.type === "finish")
    .map((event) => [event.entryId, event]));
  for (const [index, entry] of officialFinishOrder.entries()) {
    const event = finishEventsByEntry.get(entry.id);
    if (!event) continue;
    const driver = entry?.lapRecords.at(-1)?.driver || driverForLap(entry, TOTAL_LAPS);
    event.finishOrderIndex = index;
  ...
  const podium = officialFinishOrder.slice(0, 3);
  ...
  for (let time = 1; time < podiumTime; time += 1) {
    events.push({
      time,
      type: "standings",
    });
  ...
  for (const entry of racers) {
    for (const record of entry.lapRecords) {
      completionTimes.set(`${entry.id}:${record.lap}`, record.end);
    }
  }
  for (const event of events.filter((item) => item.category === "event" && item.entryId && item.lap)) {
    const completionTime = completionTimes.get(`${event.entryId}:${event.lap}`);
    if (completionTime && completionTime <= event.time) {
      event.time = Math.max(0.001, completionTime - 0.01);
