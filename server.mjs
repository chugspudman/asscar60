...
function requireLoggedIn(request) {
  const session = currentSession(request);
  if (!session?.username) {
    const error = new Error("Please log in first.");
    error.statusCode = 401;
  ...
  try {
    ...
    if (request.method === "GET" && requestedPath === "/api/draft") {
      ...
      sendJson(response, 200, {
        ...store.getDraft(session),
        unreadDraftVoteAlertIds: store.draftVoteAlertIdsForSession(session),
      });
      return;
    }

    if (request.method === "POST" && requestedPath === "/api/draft/alerts/seen") {
      const manager = requireManager(request);
      const input = await readJson(request);
      sendJson(response, 200, store.markDraftVoteAlertsSeen(
        manager.username,
        Array.isArray(input.alertIds) ? input.alertIds : [],
      ));
      return;
    }
    ...
    if (request.method === "POST" && requestedPath === "/api/draft/retention") {
      const input = await readJson(request);
      const manager = requireManager(request);
      sendJson(response, 201, store.saveOpeningDraftRetention({ ...input, teamId: manager.teamId }));
      return;
    }

    if (request.method === "POST" && requestedPath === "/api/draft/pit-coach") {
      const input = await readJson(request);
      const manager = requireManager(request);
      sendJson(response, 201, store.savePitCoachSpecialty({ ...input, teamId: manager.teamId }));
      return;
    }
    ...
    if (request.method === "GET" && requestedPath === "/api/rookie-draft") {
      const session = currentSession(request) || {};
      sendJson(response, 200, {
        ...store.getRookieDraft(store.getActiveSeason()),
        unreadDraftVoteAlertIds: store.draftVoteAlertIdsForSession(session),
      });
      return;
    }

    if (request.method === "POST" && requestedPath === "/api/rookie-draft/start") {
      const input = await readJson(request);
      const manager = requireManager(request);
      sendJson(response, 201, store.voteToStartRookieDraft({
        ...input,
    ...
    if (request.method === "POST" && requestedPath === "/api/media") {
      const manager = requireLoggedIn(request);
      sendJson(response, 201, store.createMediaEntry(manager.username, await readJson(request)));
      return;
    }

    const mediaSeenMatch = requestedPath.match(/^\/api\/media\/(\d+)\/seen$/);
    if (request.method === "POST" && mediaSeenMatch) {
      const manager = requireLoggedIn(request);
      sendJson(response, 200, store.markMediaEntrySeen(manager.username, Number(mediaSeenMatch[1])));
      return;
    }
    ...
    const mediaMatch = requestedPath.match(/^\/api\/media\/(\d+)$/);
    if (request.method === "PUT" && mediaMatch) {
      const manager = requireLoggedIn(request);
      sendJson(
        response,
        200,
        store.updateMediaEntry(manager.username, Number(mediaMatch[1]), await readJson(request)),
      );
      return;
    }
    ...
    if (request.method === "DELETE" && mediaMatch) {
      const manager = requireLoggedIn(request);
      sendJson(response, 200, store.deleteMediaEntry(manager.username, Number(mediaMatch[1])));
      return;
    }
