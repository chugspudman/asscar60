    ...
    if (nextTime > 6) {
      ...
      for (let index = 1; index < order.length; index += 1) {
        ...
        const liveOrder = runningAtTick
          .filter((entry) => entry.status === "running")
          .sort((a, b) => b.position - a.position);
        const liveAttackerIndex = liveOrder.findIndex((entry) => entry.id === attacker.id);
        if (liveAttackerIndex < 1 || liveOrder[liveAttackerIndex - 1]?.id !== defender.id) {
          continue;
        }
        ...
        const gap = defender.position - attacker.position;
        if (gap <= 0 || gap > attackerSpeed) continue;
        const pair = `${attacker.id}:${defender.id}`;
        ...
        if (attackerWinsOvertake(
          ...
          attacker.position = Math.min(TOTAL_LAPS, defender.position + defenderSpeed * 2 + 0.001);
