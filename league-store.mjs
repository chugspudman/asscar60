...
function generateRaceRecap(race, context = {}) {
  ...
  if (rookieDebuts.length && veteranSeasonDebuts.length) {
    paragraphTwoCandidates.push({ priority: 48, text: recapChoice([
    ...
    paragraphTwoCandidates.push({ priority: 34, text: recapChoice([
...
export function createLeagueStore(path = ":memory:") {
  ...
  function createMediaEntry(username, input) {
    ...
    const result = insertMediaEntry.run(title, body, username, now, now);
    insertMediaEntryRead.run(username, result.lastInsertRowid, now);
