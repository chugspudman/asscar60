...
function updateTrainingLockEligibility() {
  ...
  if (!button || !error || racerSelects.length < 2 || button.disabled && !racerSelects.every((select) => !select.disabled)) return;
  const duplicateDriver = racerSelects[0].value && racerSelects[0].value === racerSelects[1].value;
  error.textContent = duplicateDriver ? "You must select two different drivers." : "";
...
function renderDevelopment() {
  ...
  const trainingUnavailable = trainingApplied || eligibleRacers.length < 2;
  ...
  const firstChoice = choiceForSlot(0) || teamChoices[0] || null;
  const secondChoice = choiceForSlot(1) || teamChoices[1] || null;
  const firstDefaultRacer = firstChoice?.racer_id || eligibleRacers[0]?.id || "";
  const secondDefaultRacer = secondChoice?.racer_id
    || eligibleRacers.find((racer) => racer.id !== firstDefaultRacer)?.id
    || eligibleRacers[1]?.id
    || "";
  elements.upgradeOptions.innerHTML = `
    <article class="development-choice-card ${trainingUnavailable ? "unavailable" : ""} ${teamChoices.length && !trainingApplied ? "selected" : ""}">
      <p class="development-choice-line">
        Increase the
        <select data-training-stat="0" aria-label="First weekly training trait" ${trainingUnavailable ? "disabled" : ""}>
          ${statOptions(firstChoice?.option_index ?? 0)}
        </select>
        of
        <select data-training-racer="0" aria-label="First weekly training driver" ${trainingUnavailable ? "disabled" : ""}>
          ${racerOptions(firstDefaultRacer)}
        </select>
        by 1.
      </p>
      <p class="development-choice-line">
        Increase the
        <select data-training-stat="1" aria-label="Second weekly training trait" ${trainingUnavailable ? "disabled" : ""}>
          ${statOptions(secondChoice?.option_index ?? 0)}
        </select>
        of
        <select data-training-racer="1" aria-label="Second weekly training driver" ${trainingUnavailable ? "disabled" : ""}>
          ${racerOptions(secondDefaultRacer)}
        </select>
        by 1.
      </p>
      ...
      <p class="muted">${trainingApplied
        ...
        : eligibleRacers.length < 2
          ? "At least two different racers must drive this week before trainings can be locked in."
  ...
  const carApplied = Boolean(carChoice?.applied_at);
  elements.carUpgradeOptions.innerHTML = `
    <article class="development-choice-card ${carApplied ? "unavailable" : ""} ${carChoice && !carApplied ? "selected" : ""}">
      <p class="development-choice-line">
        Increase the
        <select data-car-training-stat aria-label="Weekly car upgrade trait" ${carApplied ? "disabled" : ""}>
          ${state.development.carOptions.map((stat, optionIndex) => `
            <option value="${optionIndex}" ${carChoice?.option_index === optionIndex ? "selected" : ""}>${titleCase(stat)}</option>
          `).join("")}
        </select>
        of
        <select data-upgrade-car aria-label="Weekly upgrade car" ${carApplied ? "disabled" : ""}>
          ${state.carNames[actingTeamId].map((name, carIndex) => `
            <option value="${carIndex}" ${carChoice?.car_index === carIndex ? "selected" : ""}>${actingTeam.short} ${escapeHtml(name)}</option>
          `).join("")}
        </select>
        by 1.
      </p>
      <button class="primary-button" data-car-upgrade-option ${carApplied ? "disabled" : ""}>
        ${carApplied ? "Upgrade applied" : carChoice ? "Update lock-in" : "Lock In"}
      </button>
...
elements.upgradeOptions.addEventListener("click", async (event) => {
  ...
  try {
    const response = await fetch("/api/development/choose", {
      ...
      body: JSON.stringify({
        teamId: managedTeamId(),
        trainings: [0, 1].map((slot) => ({
          racerId: racerSelects[slot]?.value,
...
elements.carUpgradeOptions.addEventListener("click", async (event) => {
  ...
  if (!button) return;
  const optionIndex = Number(elements.carUpgradeOptions.querySelector("[data-car-training-stat]")?.value);
  const carSelect = elements.carUpgradeOptions.querySelector("[data-upgrade-car]");
  button.disabled = true;
