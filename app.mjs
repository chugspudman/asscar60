...
function renderCourses() {
  elements.courseList.innerHTML = COURSES.map((course, index) => {
    ...
    const roleAverages = state.raceCenter.lapPerformance?.courseRoleAverages?.[course.name] || {};
    return `
          ...
          <div class="performance-grid compact">
            ${["Opener", "Bridge", "Closer"].map((role) => `
              <div class="performance-tile">
                ...
                <strong>${formatPerformanceTime(roleAverages[role])}</strong>
                <small>${role}</small>
              </div>
            `).join("")}
          </div>
...
function relayPerformanceMarkup(driverId, role) {
  ...
  return `
    <div class="relay-performance">
      ...
      <span class="${Number(performance.vsCourseRole?.[vsKey]) < 0 ? "good" : Number(performance.vsCourseRole?.[vsKey]) > 0 ? "bad" : ""}" title="Driver Average Vs Course/Role Average">${vsKey} ${formatPerformanceDelta(performance.vsCourseRole?.[vsKey])}</span>
    </div>`;
}
...
function racerPerformanceProfileMarkup(racer) {
  ...
  return `
    <div class="racer-performance-profile">
      ...
      <div class="performance-section">
        <small>Driver Average Vs Course/Role Average / V@@</small>
