...
function renderInitialStandings(entries = currentEntries()) {
  elements.standings.innerHTML = entries.map((entry, index) => `
    <div class="standing-row">
      ...
      <span class="entry-name"><i class="team-stripe" style="background:${entry.color}"></i>${entry.carName}</span>
      <span class="driver-name">${abbreviatedRacerName(entry.stints[0].driver.name)}</span>
  ...
  elements.championship.innerHTML = state.raceCenter.championship.map((standing) => {
    ...
    return `
      <div class="championship-row">
        ...
        <i class="team-stripe" style="background:${team.color}"></i>
...
function renderGarageCars() {
  const team = teams.find((item) => item.id === state.selectedTeamId);
  const carNames = state.carNames[team.id];
...
function renderLineupEditor() {
  const team = teams.find((item) => item.id === state.selectedTeamId);
  const selections = state.lineups[team.id];
  const carNames = state.carNames[team.id];
        ...
        return `
          <div class="stint-row">
            ...
            <label class="length-control">
              <span>Length</span>
              <input type="number" min="5" max="40" step="1" value="${selection.laps}" data-laps-slot="${slot}" aria-label="${team.short} car ${carIndex + 1} stint ${stintIndex + 1} length">
              <span>laps</span>
            </label>
          </div>`;
      }).join("")}
...
function contrastColor(hex) {
  ...
  return luminance < 0.52 ? "#ffffff" : "#18231f";
}

function lightTeamColor(hex) {
  const value = String(hex).replace("#", "");
  const channels = [0, 2, 4].map((offset) => parseInt(value.slice(offset, offset + 2), 16));
  return `rgb(${channels.map((channel) => Math.round(channel + (255 - channel) * 0.78)).join(", ")})`;
}
...
function renderBrand() {
  ...
  elements.brandEditor.innerHTML = `
    ...
    <article class="brand-card brand-color-card panel">
      <span class="brand-status">${lockText(brand.color_changed_season)}</span>
      <h2>Team color</h2>
      ...
      <div class="brand-palette">
        ${state.brandColors.map((color) => {
          ...
          const locked = brand.color_changed_season === state.raceCenter.season;
          return `
...
function updateColorEligibility() {
  ...
  button.disabled = locked || unchanged;
  message.textContent = !locked && unchanged
...
function renderDevelopment() {
  ...
  elements.upgradeOptions.innerHTML = state.development.options.map((stat, optionIndex) => {
    const unavailable = Boolean(teamChoice) || !eligibleRacers.length;
    const buttonText = teamChoice
      ? "Weekly choice used"
      : eligibleRacers.length
        ? `Choose ${titleCase(stat)}`
        : "No eligible racers";
    return `
      <article class="upgrade-card ${unavailable ? "unavailable" : ""}">
        <span class="upgrade-number">League option ${optionIndex + 1}</span>
        <h3>${titleCase(stat)} Training</h3>
        <p>Choose a racer who drove this week. Success adds +1; failure permanently reveals a cap.</p>
        <select data-upgrade-racer="${optionIndex}" aria-label="${titleCase(stat)} training racer" ${unavailable ? "disabled" : ""}>
          ${eligibleRacers.length
            ? optionMarkup(eligibleRacers)
  ...
  elements.carUpgradeOptions.innerHTML = state.development.carOptions.map(
    (stat, optionIndex) => {
      const unavailable = Boolean(carChoice);
      return `
        <article class="upgrade-card ${unavailable ? "unavailable" : ""}">
          <span class="upgrade-number">League option ${optionIndex + 1}</span>
          <h3>${titleCase(stat)} Upgrade</h3>
          <p>Add +1 ${stat} to one of this team's cars, up to the maximum rating of 10.</p>
          <select data-upgrade-car="${optionIndex}" aria-label="${titleCase(stat)} car" ${unavailable ? "disabled" : ""}>
            ${state.carNames[actingTeamId].map((name, carIndex) => `
              <option value="${carIndex}">${actingTeam.short} ${escapeHtml(name)}</option>
            `).join("")}
          </select>
          <button class="primary-button" data-car-upgrade-option="${optionIndex}" ${unavailable ? "disabled" : ""}>
            ${unavailable ? "Weekly Upgrade used" : `Choose ${titleCase(stat)}`}
          </button>
        </article>`;
    },
  ).join("");
  ...
  const trainingHistory = state.development.choices.map((choice) => {
      ...
      const description = choice.result === "improved"
      ...
      return `
        <article class="development-record">
          ...
          <span class="development-result ${choice.result}">${choice.result}</span>
        </article>`;
    });
  const carHistory = state.development.carChoices.map((choice) => {
    ...
    return `
      <article class="development-record">
        <span class="team-code">${team.short}</span>
        <p>${escapeHtml(`${team.short} ${carName} gained +1 ${titleCase(choice.stat)}.`)}</p>
        <span class="development-result improved">upgraded</span>
      </article>`;
  });
...
async function saveBrandChange({ button, element, value, teamId }) {
  ...
  try {
    ...
    renderInitialStandings();
    renderRaceControls();
    elements.brandMessage.textContent = `${titleCase(element)} updated for Season ${state.raceCenter.season}.`;
  } catch (error) {
...
elements.brandEditor.addEventListener("click", (event) => {
  ...
  elements.brandConfirmMessage.textContent = element === "color"
    ? "This element of your team brand may only be changed once per season, and will also change your main button color. Are you sure you want to make the change?"
...
elements.upgradeOptions.addEventListener("click", async (event) => {
  ...
  try {
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || "Could not apply weekly upgrade.");
    ...
    renderDevelopment();
    renderBrand();
    elements.developmentMessage.textContent = result.result === "improved"
      ? `${result.racer.name} improved ${titleCase(result.stat)} to ${result.racer[result.stat]}.`
...
elements.carUpgradeOptions.addEventListener("click", async (event) => {
  ...
  try {
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || "Could not apply car upgrade.");
    await Promise.all([loadLeagueState(), loadDevelopment()]);
    renderGarageCars();
    renderDevelopment();
    const carName = state.carNames[result.teamId][result.carIndex];
    elements.developmentMessage.textContent = `${carName} improved ${titleCase(result.stat)} to ${result.car[result.stat]}.`;
  } catch (error) {
    elements.developmentMessage.textContent = error.message;
    button.disabled = false;
  }
...
async function startApp() {
  ...
  rookieDraftInterval = setInterval(syncRookieDraft, 30_000);
...
window.addEventListener("resize", calibrateNewsTicker);
