...
export function createLeagueStore(path = ":memory:") {
  ...
  function chooseWeeklyUpgrade(teamId, trainings, week = 1) {
    ...
    const normalizedTrainings = Array.isArray(trainings)
      ? trainings.slice(0, 2).map((training, slot) => ({
        slot,
        racerId: String(training?.racerId || ""),
        optionIndex: Number(training?.optionIndex),
      }))
      : [];
    if (normalizedTrainings.length !== 2) {
      throw new Error("Choose two weekly trainings.");
    }
    if (normalizedTrainings[0].racerId === normalizedTrainings[1].racerId) {
      throw new Error("You must select two different drivers.");
    }
