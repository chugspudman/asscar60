  ...
  elements.mediaList.innerHTML = state.mediaEntries.map((entry) => {
    ...
    return `
      <article class="media-entry ${expanded ? "expanded" : ""}" data-media-entry="${entry.id}">
        <button type="button" class="media-entry-summary" data-media-toggle="${entry.id}">
          <span>
        ...
        <div class="media-entry-full" ${expanded ? "" : "hidden"}>
          ${canModify ? `
            <div class="media-entry-actions">
              <button type="button" aria-label="Edit media entry" title="Edit" data-media-edit="${entry.id}">&#9998;</button>
              <button type="button" aria-label="Delete media entry" title="Delete" data-media-delete="${entry.id}">&#128465;</button>
            </div>
          ` : ""}
...
async function loadMediaEntries() {
  ...
  state.mediaEntries = await response.json();
}
...
elements.closeMedia.addEventListener("click", returnToLeagueView);
