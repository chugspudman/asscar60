...
function renderRaceProgress(standings = [], label = "") {
  ...
  const leaderProgress = leader
    ? Number.isFinite(leader.progress)
      ? leader.progress
      : Number.isFinite(leader.raceProgress)
        ? leader.raceProgress - Math.floor(leader.raceProgress)
        : 0
    : 0;
  ...
  const progressDots = rows.map((standing) => {
    ...
    const progress = Number.isFinite(standing.progress)
      ? standing.progress
      : Number.isFinite(standing.raceProgress)
        ? standing.raceProgress - Math.floor(standing.raceProgress)
        : 0;
...
function refreshDraftVoteAlerts() {
  ...
  state.clearedDraftVoteAlertKeys = state.clearedDraftVoteAlertKeys.filter((key) => activeSet.has(key));
  updateDraftVoteAlertBadges();
}
