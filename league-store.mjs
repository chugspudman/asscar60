...
const CAR_STATS = ["speed", "handling", "durability"];
...
function statHeadroom(...ratings) {
  return ratings.reduce((sum, rating) => sum + Math.max(0, 10 - Number(rating || 0)), 0);
}

function rollPotentialWithinHeadroom(random, ratings, minimum, maximum) {
  const headroom = statHeadroom(...ratings);
  if (headroom <= 0) return 0;
  const cappedMaximum = Math.min(maximum, headroom);
  const cappedMinimum = Math.min(minimum, cappedMaximum);
  return cappedMinimum + Math.floor(random() * (cappedMaximum - cappedMinimum + 1));
}

function followingEasternMonday(parts) {
  const weekdayIndex = {
    Mon: 1,
    Tue: 2,
    Wed: 3,
  ...
  const daysUntilNextMonday = ((8 - weekdayIndex) % 7) || 7;
  return addEasternDays(parts, daysUntilNextMonday);
}

function firstRaceAtForDraft(startedAt) {
  const draftParts = easternParts(new Date(startedAt));
  return easternWallTimeToDate(followingEasternMonday(draftParts)).toISOString();
}
...
function calculateTeamTotals(raceRows, activeSeason, currentSeasonStandings = [], bonusProvider = () => ({})) {
  const totals = Object.fromEntries(teams.map((team) => [team.id, {
    seasonPoints: 0,
    franchisePoints: 0,
    goldMedals: 0,
    silverMedals: 0,
  ...
  for (const raceRow of raceRows) {
    ...
    const entryById = new Map(entries.map((entry) => [entry.id, entry]));
    for (const standing of standings) {
      if (standing.status === "dnf") continue;
      const entry = entryById.get(standing.id);
      const record = entry ? totals[entry.teamId] : null;
      if (!record) continue;
      if (standing.position === 1) record.goldMedals += 1;
      if (standing.position === 2) record.silverMedals += 1;
      if (standing.position === 3) record.bronzeMedals += 1;
    }
  }

  for (const [season, seasonRaces] of racesBySeason.entries()) {
    const standings = Number(season) === Number(activeSeason)
      ? currentSeasonStandings
      : calculateSeasonChampionships(seasonRaces, bonusProvider(season)).teams;
    for (const standing of standings) {
      const record = totals[standing.teamId];
      if (!record) continue;
      const points = Number(standing.points || 0);
      record.franchisePoints += points;
      if (Number(season) === Number(activeSeason)) record.seasonPoints = points;
    }
  }
...
function qualifierLapResult(entry, driver, course, condition, random) {
  const baseTime = basePartDuration(course.length, 10, entry.vehicle.speed);
...
export function createLeagueStore(path = ":memory:") {
  ...
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS draft_retention_selections (
      season INTEGER NOT NULL,
      team_id TEXT NOT NULL,
      veteran_retention_id TEXT NOT NULL REFERENCES racers(id),
      rookie_retention_id TEXT NOT NULL REFERENCES racers(id),
    ...
    CREATE TABLE IF NOT EXISTS trade_offers (
      ...
      offered_racer_id TEXT REFERENCES racers(id),
      requested_racer_id TEXT REFERENCES racers(id),
    ...
    CREATE TABLE IF NOT EXISTS traded_draft_pick_rights (
      season INTEGER NOT NULL,
      draft_type TEXT NOT NULL CHECK (draft_type IN ('opening')),
      ...
      created_at TEXT NOT NULL,
      PRIMARY KEY (season, draft_type, original_team_id, round_number)
    ) STRICT;

    CREATE TABLE IF NOT EXISTS traded_training_rights (
      ...
      trade_offer_id INTEGER REFERENCES trade_offers(id),
      created_at TEXT NOT NULL,
      PRIMARY KEY (week, original_team_id, slot)
    ) STRICT;

    CREATE TABLE IF NOT EXISTS traded_car_upgrade_rights (
      week INTEGER NOT NULL,
      original_team_id TEXT NOT NULL,
      slot INTEGER NOT NULL CHECK (slot >= 0),
      owner_team_id TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS pit_coach_selections (
      season INTEGER NOT NULL,
      team_id TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS season_course_schedule (
      season INTEGER NOT NULL,
      week INTEGER NOT NULL CHECK (week BETWEEN 1 AND 4),
      course_name TEXT NOT NULL,
      created_at TEXT NOT NULL,
      PRIMARY KEY (season, week),
      UNIQUE (season, course_name)
    ) STRICT;
  ...
  database.exec(`
    INSERT OR IGNORE INTO draft_retention_selections (
      season, team_id, veteran_retention_id, rookie_retention_id, selected_at
    )
    SELECT season, team_id, veteran_retention_id, rookie_retention_id, voted_at
    FROM draft_initiation_votes
    WHERE veteran_retention_id IS NOT NULL
      AND rookie_retention_id IS NOT NULL
  `);
  const tradeOfferColumns = database.prepare("PRAGMA table_info(trade_offers)").all();
  if (!tradeOfferColumns.some((column) => column.name === "offered_racer_ids_json")) {
    database.exec("ALTER TABLE trade_offers ADD COLUMN offered_racer_ids_json TEXT");
    database.exec("UPDATE trade_offers SET offered_racer_ids_json = json_array(offered_racer_id) WHERE offered_racer_ids_json IS NULL");
  }
  if (!tradeOfferColumns.some((column) => column.name === "requested_racer_ids_json")) {
    database.exec("ALTER TABLE trade_offers ADD COLUMN requested_racer_ids_json TEXT");
    database.exec("UPDATE trade_offers SET requested_racer_ids_json = json_array(requested_racer_id) WHERE requested_racer_ids_json IS NULL");
  }
  if (!tradeOfferColumns.some((column) => column.name === "offered_items_json")) {
    database.exec("ALTER TABLE trade_offers ADD COLUMN offered_items_json TEXT");
    database.exec("UPDATE trade_offers SET offered_items_json = json_array(json_object('type', 'driver', 'racerId', offered_racer_id)) WHERE offered_items_json IS NULL");
  }
  if (!tradeOfferColumns.some((column) => column.name === "requested_items_json")) {
    database.exec("ALTER TABLE trade_offers ADD COLUMN requested_items_json TEXT");
    database.exec("UPDATE trade_offers SET requested_items_json = json_array(json_object('type', 'driver', 'racerId', requested_racer_id)) WHERE requested_items_json IS NULL");
  }
  const tradeOfferTable = database.prepare(`
    SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'trade_offers'
  `).get();
  if (tradeOfferTable?.sql?.includes("offered_racer_id TEXT NOT NULL")) {
    database.exec(`
      PRAGMA foreign_keys = OFF;
      PRAGMA legacy_alter_table = ON;
      ALTER TABLE trade_offers RENAME TO trade_offers_old;
      CREATE TABLE trade_offers (
        ...
        offered_racer_id TEXT REFERENCES racers(id),
        requested_racer_id TEXT REFERENCES racers(id),
        offered_racer_ids_json TEXT,
        requested_racer_ids_json TEXT,
      ...
      INSERT INTO trade_offers (
        id, offering_team_id, receiving_team_id, offered_racer_id, requested_racer_id,
        offered_racer_ids_json, requested_racer_ids_json, offered_items_json, requested_items_json,
        status, created_at, resolved_at
      )
      SELECT id, offering_team_id, receiving_team_id, offered_racer_id, requested_racer_id,
        offered_racer_ids_json, requested_racer_ids_json, offered_items_json, requested_items_json,
        status, created_at, resolved_at
      FROM trade_offers_old;
      DROP TABLE trade_offers_old;
      PRAGMA legacy_alter_table = OFF;
      PRAGMA foreign_keys = ON;
    `);
  }
  function repairTradeRightsForeignKey(tableName, createSql, columns) {
    const foreignKeys = database.prepare(`PRAGMA foreign_key_list(${tableName})`).all();
    if (!foreignKeys.some((foreignKey) => foreignKey.table === "trade_offers_old")) return;
    const temporaryTableName = `${tableName}_new`;
    database.exec("PRAGMA foreign_keys = OFF");
    database.exec(createSql.replace(`CREATE TABLE ${tableName}`, `CREATE TABLE ${temporaryTableName}`));
    database.exec(`
      INSERT INTO ${temporaryTableName} (${columns.join(", ")})
      SELECT ${columns.join(", ")}
      FROM ${tableName}
    `);
  ...
  repairTradeRightsForeignKey(
    "traded_draft_pick_rights",
    `CREATE TABLE traded_draft_pick_rights (
      season INTEGER NOT NULL,
      draft_type TEXT NOT NULL CHECK (draft_type IN ('opening')),
      original_team_id TEXT NOT NULL,
      round_number INTEGER NOT NULL CHECK (round_number BETWEEN 1 AND 6),
    ...
    ["season", "draft_type", "original_team_id", "round_number", "owner_team_id", "trade_offer_id", "created_at"],
  );
  repairTradeRightsForeignKey(
    "traded_training_rights",
    `CREATE TABLE traded_training_rights (
      week INTEGER NOT NULL,
      original_team_id TEXT NOT NULL,
      slot INTEGER NOT NULL CHECK (slot >= 0),
      owner_team_id TEXT NOT NULL,
    ...
    ["week", "original_team_id", "slot", "owner_team_id", "trade_offer_id", "created_at"],
  );
  repairTradeRightsForeignKey(
    "traded_car_upgrade_rights",
    `CREATE TABLE traded_car_upgrade_rights (
      week INTEGER NOT NULL,
      original_team_id TEXT NOT NULL,
      slot INTEGER NOT NULL CHECK (slot >= 0),
      owner_team_id TEXT NOT NULL,
  ...
  if (!weeklyChoiceColumns.some((column) => column.name === "slot")) {
    database.exec(`
      PRAGMA foreign_keys = OFF;
      ALTER TABLE weekly_upgrade_choices RENAME TO weekly_upgrade_choices_old;
      CREATE TABLE weekly_upgrade_choices (
        week INTEGER NOT NULL REFERENCES weekly_upgrade_weeks(week),
        team_id TEXT NOT NULL,
        slot INTEGER NOT NULL DEFAULT 0 CHECK (slot BETWEEN 0 AND 1),
        option_index INTEGER NOT NULL CHECK (option_index BETWEEN 0 AND 2),
        racer_id TEXT NOT NULL REFERENCES racers(id),
        stat TEXT NOT NULL,
        result TEXT NOT NULL CHECK (result IN ('improved', 'capped')),
      ...
      INSERT INTO weekly_upgrade_choices (
        week, team_id, slot, option_index, racer_id, stat, result, created_at, applied_at
      )
      SELECT week, team_id, 0, option_index, racer_id, stat, result, created_at, applied_at
      FROM weekly_upgrade_choices_old;
      DROP TABLE weekly_upgrade_choices_old;
      PRAGMA foreign_keys = ON;
    `);
  }
  const weeklyChoiceTable = database.prepare(`
    SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'weekly_upgrade_choices'
  `).get();
  if (weeklyChoiceTable?.sql?.includes("slot BETWEEN 0 AND 1")) {
    database.exec(`
      PRAGMA foreign_keys = OFF;
      ALTER TABLE weekly_upgrade_choices RENAME TO weekly_upgrade_choices_old;
      CREATE TABLE weekly_upgrade_choices (
      ...
      SELECT week, team_id, slot, option_index, racer_id, stat, result, created_at, applied_at
      FROM weekly_upgrade_choices_old;
      DROP TABLE weekly_upgrade_choices_old;
      PRAGMA foreign_keys = ON;
    `);
  ...
  if (!weeklyCarChoiceColumns.some((column) => column.name === "slot")) {
    database.exec(`
      PRAGMA foreign_keys = OFF;
      ALTER TABLE weekly_car_upgrade_choices RENAME TO weekly_car_upgrade_choices_old;
      CREATE TABLE weekly_car_upgrade_choices (
        week INTEGER NOT NULL REFERENCES weekly_car_upgrade_weeks(week),
        team_id TEXT NOT NULL,
        slot INTEGER NOT NULL DEFAULT 0 CHECK (slot >= 0),
        option_index INTEGER NOT NULL CHECK (option_index BETWEEN 0 AND 2),
      ...
      INSERT INTO weekly_car_upgrade_choices (
        week, team_id, slot, option_index, car_index, stat, created_at, applied_at
      )
      SELECT week, team_id, 0, option_index, car_index, stat, created_at, applied_at
      FROM weekly_car_upgrade_choices_old;
      DROP TABLE weekly_car_upgrade_choices_old;
      PRAGMA foreign_keys = ON;
    `);
  }
  database.exec(`
  ...
  function seedOpeningDraftPool(seed = randomBytes(4).readUInt32BE(0)) {
    ...
    names.forEach((name, index) => {
      ...
      const overtaking = rating();
      const stamina = rating();
      const racerId = `opening-draft-${seed}-${index + 1}`;
      const legacyRating = 5;
      insertRacer.run(
        racerId,
        name,
        legacyRating,
        ...
        legacyRating,
        legacyRating,
        rollPotentialWithinHeadroom(random, [control, overtaking, stamina], 1, 5),
  ...
  try {
    ...
    freeAgentNames.forEach((name, index) => {
      ...
      insertRacer.run(
        ...
        legacyRating,
        legacyRating,
        rollPotentialWithinHeadroom(freeAgentRandom, [control, overtaking, stamina], 1, 5),
  ...
  `);
  const readDraftRetentionSelections = database.prepare(`
    SELECT season, team_id, veteran_retention_id, rookie_retention_id, selected_at
    FROM draft_retention_selections
    WHERE season = ?
    ORDER BY selected_at, team_id
  `);
  const upsertDraftRetentionSelection = database.prepare(`
    INSERT INTO draft_retention_selections (
      season, team_id, veteran_retention_id, rookie_retention_id, selected_at
    ) VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(season, team_id) DO UPDATE SET
      veteran_retention_id = excluded.veteran_retention_id,
      rookie_retention_id = excluded.rookie_retention_id,
  ...
  `);
  const readTradeRacers = database.prepare(`
    SELECT id, name, team_id FROM racers
  `);
  ...
  const insertTradeOffer = database.prepare(`
    INSERT INTO trade_offers (
      ...
      requested_racer_id, offered_racer_ids_json, requested_racer_ids_json,
      offered_items_json, requested_items_json, status, created_at, resolved_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, NULL)
  `);
  ...
  const insertTradeAlertRead = database.prepare(`
    ...
    VALUES (?, ?, ?)
  `);
  const readDraftPickRightsForSeason = database.prepare(`
    SELECT *
    FROM traded_draft_pick_rights
    WHERE season = ? AND draft_type = 'opening'
    ORDER BY round_number, original_team_id
  `);
  const upsertDraftPickRight = database.prepare(`
    INSERT INTO traded_draft_pick_rights (
      season, draft_type, original_team_id, round_number,
      owner_team_id, trade_offer_id, created_at
    ) VALUES (?, 'opening', ?, ?, ?, ?, ?)
    ON CONFLICT(season, draft_type, original_team_id, round_number)
    DO UPDATE SET
      owner_team_id = excluded.owner_team_id,
      trade_offer_id = excluded.trade_offer_id,
      created_at = excluded.created_at
  `);
  const readTrainingRightsForWeek = database.prepare(`
    SELECT *
    FROM traded_training_rights
    WHERE week = ?
    ORDER BY original_team_id, slot
  `);
  const upsertTrainingRight = database.prepare(`
    INSERT INTO traded_training_rights (
      week, original_team_id, slot, owner_team_id, trade_offer_id, created_at
    ) VALUES (?, ?, ?, ?, ?, ?)
    ON CONFLICT(week, original_team_id, slot)
    DO UPDATE SET
      owner_team_id = excluded.owner_team_id,
      trade_offer_id = excluded.trade_offer_id,
      created_at = excluded.created_at
  `);
  const readCarUpgradeRightsForWeek = database.prepare(`
    SELECT *
    FROM traded_car_upgrade_rights
    WHERE week = ?
    ORDER BY original_team_id, slot
  `);
  const upsertCarUpgradeRight = database.prepare(`
    INSERT INTO traded_car_upgrade_rights (
      week, original_team_id, slot, owner_team_id, trade_offer_id, created_at
    ) VALUES (?, ?, ?, ?, ?, ?)
    ON CONFLICT(week, original_team_id, slot)
    DO UPDATE SET
  ...
  const insertDraftVoteAlertRead = database.prepare(`
    ...
    VALUES (?, ?, ?)
  `);
  const readPitCoachSelections = database.prepare(`
    SELECT season, team_id, specialty_key, coach_name, selected_at
    FROM pit_coach_selections
    WHERE season = ?
    ORDER BY selected_at, team_id
  `);
  const readPitCoachSelection = database.prepare(`
    SELECT season, team_id, specialty_key, coach_name, selected_at
    FROM pit_coach_selections
    WHERE season = ? AND team_id = ?
  `);
  const upsertPitCoachSelection = database.prepare(`
    INSERT INTO pit_coach_selections (season, team_id, specialty_key, coach_name, selected_at)
    VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(season, team_id) DO UPDATE SET
  ...
  const readTransactions = database.prepare(`
    ...
    LIMIT 100
  `);
  const readTeamHistoryDraftPicks = database.prepare(`
    SELECT racer_id, team_id FROM draft_picks ORDER BY pick_number
  `);
  const readTeamHistoryRookiePicks = database.prepare(`
    SELECT racer_id, team_id FROM rookie_draft_picks ORDER BY season, pick_number
  `);
  const readTeamHistoryTransactions = database.prepare(`
    SELECT type, team_id, other_team_id, acquired_racer_id, moved_racer_id
    FROM transactions
    ORDER BY id
  `);
  ...
  `);
  const readGrowthRows = database.prepare(`
    SELECT stat, remaining FROM racer_growth
    WHERE racer_id = ?
  `);
  const updateGrowthRemaining = database.prepare(`
    UPDATE racer_growth SET remaining = ?
    WHERE racer_id = ? AND stat = ?
  `);
  ...
  `);
  const setRacerPotential = database.prepare("UPDATE racers SET potential = ? WHERE id = ?");
  ...
  `);
  const readTeamUpgradeChoices = database.prepare(`
    SELECT * FROM weekly_upgrade_choices WHERE week = ? AND team_id = ?
    ORDER BY slot
  `);
  const readAppliedTeamUpgradeChoice = database.prepare(`
    SELECT * FROM weekly_upgrade_choices
    WHERE week = ? AND team_id = ? AND applied_at IS NOT NULL
    LIMIT 1
  `);
  const deletePendingTeamUpgradeChoices = database.prepare(`
    DELETE FROM weekly_upgrade_choices
    WHERE week = ? AND team_id = ? AND applied_at IS NULL
  `);
  const insertUpgradeChoice = database.prepare(`
    INSERT INTO weekly_upgrade_choices (
      week, team_id, slot, option_index, racer_id, stat, result, created_at, applied_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL)
    ON CONFLICT(week, team_id, slot) DO UPDATE SET
  ...
  const markUpgradeChoiceApplied = database.prepare(`
    ...
    WHERE week = ? AND team_id = ? AND slot = ?
  `);
  ...
  const readCarUpgradeChoices = database.prepare(`
    ...
    ORDER BY created_at, slot
  `);
  const readTeamCarUpgradeChoices = database.prepare(`
  ...
  `);
  const deletePendingTeamCarUpgradeChoices = database.prepare(`
    DELETE FROM weekly_car_upgrade_choices
    WHERE week = ? AND team_id = ? AND applied_at IS NULL
  `);
  ...
  `);
  const readCourseSchedule = database.prepare(`
    SELECT season, week, course_name, created_at
    FROM season_course_schedule
    WHERE season = ?
    ORDER BY week
  `);
  const insertCourseSchedule = database.prepare(`
    INSERT OR IGNORE INTO season_course_schedule (
      season, week, course_name, created_at
    ) VALUES (?, ?, ?, ?)
  `);
  const readSeasonRaceCourses = database.prepare(`
    SELECT week, course_name
    FROM races
    WHERE season = ?
    GROUP BY week
    ORDER BY week
  ...
  `);
  const readAppMigration = database.prepare("SELECT key FROM app_migrations WHERE key = ?");
  const insertAppMigration = database.prepare("INSERT OR IGNORE INTO app_migrations (key, applied_at) VALUES (?, ?)");
  const deleteRaceLaps = database.prepare("DELETE FROM race_laps");
  ...
  const deletePendingCarUpgradeChoicesForSeason = database.prepare(
    "DELETE FROM weekly_car_upgrade_choices WHERE week BETWEEN ? AND ? AND applied_at IS NULL",
  );
  ...
  function ensureGrowthRows() {
    for (const racer of readAllRacersForGrowth.all()) {
      const growthRows = readGrowthRows.all(racer.id);
      if (countGrowthRows.get(racer.id).count !== TRAINABLE_STATS.length) {
      ...
      const currentGrowthRows = readGrowthRows.all(racer.id);
      for (const row of currentGrowthRows) {
        if (!TRAINABLE_STATS.includes(row.stat)) continue;
        const maxRemaining = Math.max(0, 10 - Number(racer[row.stat] || 0));
        const remaining = Math.min(Number(row.remaining || 0), maxRemaining);
        if (remaining !== row.remaining) updateGrowthRemaining.run(remaining, racer.id, row.stat);
        remainingPotential += remaining;
      }
      if (remainingPotential !== racer.potential) setRacerPotential.run(remainingPotential, racer.id);
    }
  }
  ...
  function applyPendingDevelopmentWeek(week, appliedAt = new Date().toISOString()) {
    for (const choice of readPendingUpgradeChoices.all(week)) {
      ...
      if (!racer || racer.team_id !== choice.team_id) {
        markUpgradeChoiceApplied.run("capped", appliedAt, choice.week, choice.team_id, choice.slot);
        continue;
      }
      ...
      markUpgradeChoiceApplied.run(result, appliedAt, choice.week, choice.team_id, choice.slot);
    }
    ...
    for (const choice of readPendingCarUpgradeChoices.all(week)) {
      ...
      markCarUpgradeChoiceApplied.run(appliedAt, choice.week, choice.team_id, choice.slot);
    }
  }
  ...
  function developmentSeasonForWeek(week) {
    return Math.floor((Number(week) - 1) / WEEKS_PER_SEASON) + 1;
  }

  function developmentChoicesOpenForWeek(week) {
    return countSeasonRaces.get(developmentSeasonForWeek(week)).count > 0;
  }

  function assertDevelopmentSeasonStarted(week) {
    if (!developmentChoicesOpenForWeek(week)) {
      throw new Error("Training and Upgrade choices open once the first race of the season has begun.");
    }
  }
  ...
  function getDraft(session = {}) {
    ...
    const retentionSelections = readDraftRetentionSelections.all(season);
    const retentionRequired = season > 1;
    const coachRequired = true;
    const retentionByTeam = Object.fromEntries(retentionSelections.map((selection) => [
      selection.team_id,
      {
        veteranRacerId: selection.veteran_retention_id,
        rookieRacerId: selection.rookie_retention_id,
      },
    ]));
    const coachSelections = readPitCoachSelections.all(season);
    const coachByTeam = Object.fromEntries(coachSelections.map((selection) => [
      selection.team_id,
      selection,
    ]));
    ...
    const initiation = {
      ...
      retentionRequired,
      retentionCount: teams.filter((team) => {
        const selected = retentionByTeam[team.id];
        return selected?.veteranRacerId && selected?.rookieRacerId;
      }).length,
      retentionComplete: !retentionRequired || teams.every((team) => {
        const selected = retentionByTeam[team.id];
        return selected?.veteranRacerId && selected?.rookieRacerId;
      }),
      retentionOptions,
      coachRequired,
      coachCount: coachSelections.length,
      coachComplete: !coachRequired || coachSelections.length >= teams.length,
      pitCoachOptions: PIT_COACH_SPECIALTIES,
      pitCoachSelection: session.teamId && coachByTeam[session.teamId]
        ? {
          specialtyKey: coachByTeam[session.teamId].specialty_key,
          coachName: coachByTeam[session.teamId].coach_name,
          selectedAt: coachByTeam[session.teamId].selected_at,
        }
        : null,
    };
    if (!draft) return {
    ...
    return {
      ...
      pickRights: readDraftPickRightsForSeason.all(getActiveSeason()).map((right) => ({
        season: right.season,
        originalTeamId: right.original_team_id,
        round: right.round_number,
        ownerTeamId: right.owner_team_id,
      })),
      initiation,
  ...
  function eligibleSeasonEventCandidates(season, random) {
    ...
    const asscastAllowed = !seriesStarted;
    const candidates = [];
    ...
    const bodyEnvyEligible = racesRun >= 10 && racesRun < SEASON_RACES;
    const bodyTeam = bodyEnvyEligible
      ? randomItem(teams.filter((item) => signed.filter((racer) => racer.team_id === item.id).length >= 4), random)
      : null;
  ...
  function getInMemoriam() {
    ...
    const allCareerRaces = readAllRacesForCareer.all()
      .filter((race) => race.id !== readRuntime.get().active_race_id);
    for (const lap of readCompletedRaceLaps.all()) {
      const career = careerByRacer.get(lap.racer_id) || { laps: 0, podiums: 0 };
      career.laps += 1;
      careerByRacer.set(lap.racer_id, career);
    }
    for (const raceRow of allCareerRaces) {
      ...
      const podiumRacers = new Set();
      for (const entry of entries) {
        if ((positionByEntry.get(entry.id) || Infinity) > 3) continue;
        for (const stint of entry.stints || []) {
          podiumRacers.add(stint.driver.id);
        }
      }
      for (const racerId of podiumRacers) {
        const career = careerByRacer.get(racerId) || { laps: 0, podiums: 0 };
        career.podiums += 1;
  ...
  function prepareOpeningDraftRosters(season = getActiveSeason()) {
    if (season <= 1) return;
    const retentionSelections = readDraftRetentionSelections.all(season);
    if (retentionSelections.length < teams.length) {
      throw new Error("All teams must select retainers before the opening draft can begin.");
    }
    for (const team of teams) {
      const retention = retentionSelections.find((item) => item.team_id === team.id);
      validateOpeningDraftRetention(
        team.id,
        retention?.veteran_retention_id,
        retention?.rookie_retention_id,
        season,
      );
  ...
  function openingDraftPrereqsComplete(season = getActiveSeason()) {
    const votes = readDraftInitiationVotes.all(season);
    if (votes.length < teams.length) return false;
    if (season > 1) {
      const retentionSelections = readDraftRetentionSelections.all(season);
      const retainersComplete = teams.every((team) => {
        const retention = retentionSelections.find((item) => item.team_id === team.id);
        return retention?.veteran_retention_id && retention?.rookie_retention_id;
      });
      if (!retainersComplete) return false;
    }
    if (readPitCoachSelections.all(season).length < teams.length) return false;
    return true;
  }

  function maybeStartOpeningDraftAfterPrereqs({
    rounds, poolSize = 60, seed = Date.now(), session = {},
  } = {}) {
    if (readDraftState.get()) return getDraft(session);
    const season = getActiveSeason();
    if (!openingDraftPrereqsComplete(season)) return getDraft(session);
    startDraft({
    ...
    try {
      ...
      names.forEach((name, index) => {
        ...
        insertOrReactivateGeneratedRacer(
          ...
          legacyRating,
          legacyRating,
          rollPotentialWithinHeadroom(random, [control, overtaking, stamina], 4, 6),
          notes[Math.floor(random() * notes.length)],
  ...
  function autoReleaseCandidate(teamId, season) {
    ...
    return pool.sort((a, b) => (
      (a.control + a.overtaking + a.stamina + a.potential)
      - (b.control + b.overtaking + b.stamina + b.potential)
      || a.control - b.control
      || a.overtaking - b.overtaking
      || a.stamina - b.stamina
  ...
  function voteToStartDraft({
    ...
    return maybeStartOpeningDraftAfterPrereqs({ rounds, poolSize, seed, session: { teamId } });
  }

  function saveOpeningDraftRetention({
    teamId,
    veteranRetentionId,
    rookieRetentionId,
    poolSize = 60,
    ...
    if (!teams.some((team) => team.id === teamId)) throw new Error("Unknown team.");
    const season = getActiveSeason();
    if (season <= 1) throw new Error("Retainers are only selected after the first season.");
    ...
    upsertDraftRetentionSelection.run(
      season,
      teamId,
    ...
    return maybeStartOpeningDraftAfterPrereqs({ poolSize, seed, session: { teamId } });
  }

  function savePitCoachSpecialty({
    teamId,
    specialtyKey,
    poolSize = 60,
    seed = Date.now(),
  } = {}) {
    if (readDraftState.get()) return getDraft({ teamId });
    if (!teams.some((team) => team.id === teamId)) throw new Error("Unknown team.");
    const season = getActiveSeason();
  ...
  function makeDraftPick(racerId) {
    ...
    const roundNumber = Math.floor(picks.length / teams.length) + 1;
    const originalTeamId = snakeTeamId(picks.length, openingDraftOrder(getActiveSeason()));
    const teamId = draftPickOwner(originalTeamId, roundNumber, getActiveSeason());
  ...
  function unreadTradeAlertIds(username, teamId, offers = readTradeOffers.all()) {
    if (!username) return tradeAlertIdsForTeam(offers, teamId);
  ...
  function parseTradeRacerIds(offer, side) {
    const json = side === "offered"
      ? offer.offered_racer_ids_json
      : offer.requested_racer_ids_json;
    const fallback = side === "offered"
    ...
    try {
      ...
      // Fall back to the legacy one-for-one columns below.
    }
    return fallback ? [String(fallback)] : [];
  }

  function parseTradeItems(offer, side) {
    const json = side === "offered" ? offer.offered_items_json : offer.requested_items_json;
    try {
      const items = JSON.parse(json || "[]");
      if (Array.isArray(items) && items.length) return items;
    } catch {
      // Fall back to legacy driver-only data below.
    }
    return parseTradeRacerIds(offer, side).map((racerId) => ({ type: "driver", racerId }));
  }

  function currentDevelopmentWeekKey() {
    const season = getActiveSeason();
    const racesRun = countSeasonRaces.get(season).count;
    const week = Math.min(WEEKS_PER_SEASON, Math.floor(racesRun / RACES_PER_WEEK) + 1);
    return seasonWeekKey(season, week);
  }

  function openingDraftSeasonForTrade() {
    const season = getActiveSeason();
    const draft = readDraftState.get();
    const racesRun = countSeasonRaces.get(season).count;
    if (draft && draft.status !== "complete") return season;
    if (season > 1 && racesRun === 0 && !draft) return season;
    return season + 1;
  }

  function weeklyRightsForTeam(week, baseSlots, overrides, teamId) {
    const rights = [];
    for (const originalTeam of teams) {
      for (let slot = 0; slot < baseSlots; slot += 1) {
        const override = overrides.find((row) => (
          row.original_team_id === originalTeam.id && Number(row.slot) === slot
        ));
        rights.push({
          week,
          originalTeamId: originalTeam.id,
          slot,
          ownerTeamId: override?.owner_team_id || originalTeam.id,
        });
      }
    }
    return rights.filter((right) => right.ownerTeamId === teamId);
  }

  function trainingRightsForTeam(teamId, week = currentDevelopmentWeekKey()) {
    return weeklyRightsForTeam(
      week,
      BASE_WEEKLY_TRAINING_SLOTS,
      readTrainingRightsForWeek.all(week),
      teamId,
    );
  }

  function carUpgradeRightsForTeam(teamId, week = currentDevelopmentWeekKey()) {
    return weeklyRightsForTeam(
      week,
      BASE_WEEKLY_CAR_UPGRADE_SLOTS,
      readCarUpgradeRightsForWeek.all(week),
      teamId,
    );
  }

  function nextDraftPickRight(teamId, round, season = openingDraftSeasonForTrade()) {
    const existing = readDraftPickRightsForSeason.all(season);
    const byOriginal = new Map(existing.map((row) => [
      `${row.original_team_id}:${row.round_number}`,
      row.owner_team_id,
    ]));
    const candidates = [
    ...
    return candidates
      .map((team) => ({
        ...
        round,
        ownerTeamId: byOriginal.get(`${team.id}:${round}`) || team.id,
      }))
      .find((right) => right.ownerTeamId === teamId) || null;
  }

  function draftPickOwner(originalTeamId, round, season = getActiveSeason()) {
    const right = readDraftPickRightsForSeason.all(season)
      .find((row) => row.original_team_id === originalTeamId && Number(row.round_number) === round);
    return right?.owner_team_id || originalTeamId;
  }

  function assertTradeAssetAvailable(teamId, item) {
    if (item.type === "training") {
      const needed = item.amount === "both" ? 2 : 1;
      if (trainingRightsForTeam(teamId).length < needed) {
        throw new Error("That team does not have enough next trainings to trade.");
      }
    }
    if (item.type === "upgrade" && carUpgradeRightsForTeam(teamId).length < 1) {
      throw new Error("That team does not have a next upgrade to trade.");
    }
    if (item.type === "draft_pick") {
      const draft = readDraftState.get();
      if (draft?.status === "active") {
        throw new Error("Draft picks cannot be traded during an active opening draft.");
      }
      if (!nextDraftPickRight(teamId, item.round)) {
        throw new Error("That team does not own that draft pick.");
      }
    }
  }

  function assertTeamTradeItemsAvailable(teamId, items) {
    const trainingSlotsRequested = items.reduce((sum, item) => (
      item.type === "training" ? sum + (item.amount === "both" ? 2 : 1) : sum
    ), 0);
    if (trainingSlotsRequested > trainingRightsForTeam(teamId).length) {
      throw new Error("That team does not have enough next trainings to trade.");
    }
    const upgradeSlotsRequested = items.filter((item) => item.type === "upgrade").length;
    if (upgradeSlotsRequested > carUpgradeRightsForTeam(teamId).length) {
      throw new Error("That team does not have enough next upgrades to trade.");
    }
    items.forEach((item) => assertTradeAssetAvailable(teamId, item));
  }

  function transferTrainingRights(fromTeamId, toTeamId, count, tradeOfferId, now) {
    const week = currentDevelopmentWeekKey();
    const rights = trainingRightsForTeam(fromTeamId, week).slice(0, count);
    if (rights.length < count) throw new Error("A traded training opportunity is no longer available.");
    for (const right of rights) {
      upsertTrainingRight.run(
        week,
        right.originalTeamId,
        right.slot,
        toTeamId,
        tradeOfferId,
        now,
    ...
    deletePendingTeamUpgradeChoices.run(week, fromTeamId);
    deletePendingTeamUpgradeChoices.run(week, toTeamId);
  }

  function transferCarUpgradeRights(fromTeamId, toTeamId, tradeOfferId, now) {
    const week = currentDevelopmentWeekKey();
    const right = carUpgradeRightsForTeam(fromTeamId, week)[0];
    if (!right) throw new Error("A traded upgrade opportunity is no longer available.");
    upsertCarUpgradeRight.run(
      week,
      right.originalTeamId,
      right.slot,
      toTeamId,
    ...
    deletePendingTeamCarUpgradeChoices.run(week, fromTeamId);
    deletePendingTeamCarUpgradeChoices.run(week, toTeamId);
  }

  function transferDraftPickRight(fromTeamId, toTeamId, round, tradeOfferId, now) {
    const season = openingDraftSeasonForTrade();
    const right = nextDraftPickRight(fromTeamId, round, season);
    if (!right) throw new Error("A traded draft pick is no longer available.");
    upsertDraftPickRight.run(
      season,
      right.originalTeamId,
      round,
      toTeamId,
  ...
  function transferAcceptedTradeItems(fromTeamId, toTeamId, items, tradeOfferId, now) {
    for (const item of items) {
      if (item.type === "training") {
        transferTrainingRights(fromTeamId, toTeamId, item.amount === "both" ? 2 : 1, tradeOfferId, now);
      }
      if (item.type === "upgrade") {
        transferCarUpgradeRights(fromTeamId, toTeamId, tradeOfferId, now);
      }
      if (item.type === "draft_pick") {
        transferDraftPickRight(fromTeamId, toTeamId, item.round, tradeOfferId, now);
      }
    }
  }

  function normalizeTradeItems(items, fallbackRacerIds = []) {
    const rawItems = Array.isArray(items) && items.length
      ? items
      : fallbackRacerIds.map((racerId) => ({ type: "driver", racerId }));
    const seen = new Set();
    return rawItems.slice(0, MAX_TRADE_ITEMS_PER_SIDE).map((item) => {
      if (item?.type === "driver") {
        const racerId = String(item.racerId || "");
        if (!racerId) throw new Error("Choose a driver for each driver trade item.");
        return { type: "driver", racerId };
      }
      if (item?.type === "training") {
        const amount = item.amount === "both" ? "both" : "single";
        return { type: "training", amount };
      }
      if (item?.type === "upgrade") return { type: "upgrade" };
      if (item?.type === "draft_pick") {
        const round = Number(item.round);
        if (!Number.isInteger(round) || round < 1 || round > 6) {
          throw new Error("Draft pick trade items must choose rounds 1 through 6.");
        }
        return { type: "draft_pick", round };
      }
      throw new Error("Unknown trade item.");
    }).filter((item) => {
  ...
  function tradeItemKey(item) {
    if (item.type === "driver") return `driver:${item.racerId}`;
    if (item.type === "training") return `training:${item.amount}`;
    if (item.type === "upgrade") return "upgrade";
    if (item.type === "draft_pick") return `draft_pick:${item.round}`;
    return item.type;
  }

  function tradeItemLabel(item, racers = new Map()) {
    if (item.type === "driver") return `Driver: ${racers.get(item.racerId)?.name || item.racerId}`;
    if (item.type === "training") return `Next training: ${item.amount === "both" ? "Both" : "Single"}`;
    if (item.type === "upgrade") return "Next Upgrade";
    if (item.type === "draft_pick") return `Opening Draft Pick: ${ordinal(item.round)} round`;
    return "Unknown item";
  }
  ...
  function tradeAvailability(offers = readTradeOffers.all()) {
    ...
    return Object.fromEntries(
      Object.entries(unavailable).map(([teamId, keys]) => [teamId, [...new Set(keys)]]),
    );
  }

  function tradableItemKeysByTeam() {
    const developmentWeek = currentDevelopmentWeekKey();
    const draftSeason = openingDraftSeasonForTrade();
    return Object.fromEntries(teams.map((team) => {
      const keys = [];
      if (trainingRightsForTeam(team.id, developmentWeek).length >= 1) keys.push("training:single");
      if (trainingRightsForTeam(team.id, developmentWeek).length >= 2) keys.push("training:both");
      if (carUpgradeRightsForTeam(team.id, developmentWeek).length >= 1) keys.push("upgrade");
      for (let round = 1; round <= 6; round += 1) {
        if (nextDraftPickRight(team.id, round, draftSeason)) keys.push(`draft_pick:${round}`);
      }
      return [team.id, keys];
    }));
  }

  function hydrateTradeOffers(offers) {
    const racers = new Map(readTradeRacers.all().map((racer) => [racer.id, racer]));
    return offers.map((offer) => {
      const offeredRacerIds = parseTradeRacerIds(offer, "offered");
      const requestedRacerIds = parseTradeRacerIds(offer, "requested");
      const offeredItems = parseTradeItems(offer, "offered");
      const requestedItems = parseTradeItems(offer, "requested");
      const offeredRacers = offeredRacerIds
        .map((id) => racers.get(id))
        .filter(Boolean)
        .map((racer) => ({ id: racer.id, name: racer.name, teamId: racer.team_id }));
      const requestedRacers = requestedRacerIds
        .map((id) => racers.get(id))
        .filter(Boolean)
        .map((racer) => ({ id: racer.id, name: racer.name, teamId: racer.team_id }));
      const offeredNames = offeredRacers.map((racer) => racer.name).join(", ");
      const requestedNames = requestedRacers.map((racer) => racer.name).join(", ");
      return {
        ...offer,
        offeredRacerIds,
        requestedRacerIds,
        offeredRacers,
        requestedRacers,
        offered_racer_name: offeredNames,
        requested_racer_name: requestedNames,
        offeredItems,
        requestedItems,
        offeredItemLabels: offeredItems.map((item) => tradeItemLabel(item, racers)),
        requestedItemLabels: requestedItems.map((item) => tradeItemLabel(item, racers)),
      };
    });
  }
  ...
  function getTransactions(session = {}) {
    const draft = readDraftState.get();
    const offers = hydrateTradeOffers(readTradeOffers.all());
    return {
      ...
      unavailableTradeItemsByTeam: tradeAvailability(readTradeOffers.all()),
      tradableTradeItemsByTeam: tradableItemKeysByTeam(),
    };
  }
  ...
  function getRacerDirectory() {
    const careerByRacer = new Map();
    const teamHistoryByRacer = new Map();
    const addTeamHistory = (racerId, teamId) => {
      if (!racerId || !teamId || !teams.some((team) => team.id === teamId)) return;
      if (!teamHistoryByRacer.has(racerId)) teamHistoryByRacer.set(racerId, []);
      const teamIds = teamHistoryByRacer.get(racerId);
    ...
    for (const pick of readTeamHistoryDraftPicks.all()) {
      addTeamHistory(pick.racer_id, pick.team_id);
    }
    for (const pick of readTeamHistoryRookiePicks.all()) {
      addTeamHistory(pick.racer_id, pick.team_id);
    }
    for (const transaction of readTeamHistoryTransactions.all()) {
      if (transaction.type === "trade") {
        addTeamHistory(transaction.acquired_racer_id, transaction.other_team_id);
        addTeamHistory(transaction.acquired_racer_id, transaction.team_id);
        addTeamHistory(transaction.moved_racer_id, transaction.team_id);
        addTeamHistory(transaction.moved_racer_id, transaction.other_team_id);
      } else {
        addTeamHistory(transaction.acquired_racer_id, transaction.team_id);
        addTeamHistory(transaction.moved_racer_id, transaction.team_id);
      }
    ...
    for (const raceRow of allCareerRaces) {
      ...
      for (const entry of entries) {
        ...
        for (const stint of entry.stints) {
          const racerId = stint.driver.id;
          addTeamHistory(racerId, entry.teamId);
    ...
    const withCareer = (racer) => ({
      ...publicRacer(racer),
      previousTeamIds: (teamHistoryByRacer.get(racer.id) || [])
        .filter((teamId) => teamId !== racer.team_id),
  ...
  function proposeTrade(input) {
    ...
    const offeredIds = Array.isArray(input.offeredRacerIds)
      ? input.offeredRacerIds.map(String)
      : [String(input.offeredRacerId || "")].filter(Boolean);
    const requestedIds = Array.isArray(input.requestedRacerIds)
      ? input.requestedRacerIds.map(String)
      : [String(input.requestedRacerId || "")].filter(Boolean);
    const offeredItems = normalizeTradeItems(input.offeredItems, offeredIds);
    const requestedItems = normalizeTradeItems(input.requestedItems, requestedIds);
    if (!offeredItems.length) throw new Error("Choose at least one item to offer.");
    if (!requestedItems.length) throw new Error("Choose at least one item to request.");
    const uniqueOfferedIds = [...new Set(offeredIds.filter(Boolean))];
    const uniqueRequestedIds = [...new Set(requestedIds.filter(Boolean))];
    for (const item of offeredItems) {
      if (item.type === "driver" && !uniqueOfferedIds.includes(item.racerId)) uniqueOfferedIds.push(item.racerId);
    }
    for (const item of requestedItems) {
      if (item.type === "driver" && !uniqueRequestedIds.includes(item.racerId)) uniqueRequestedIds.push(item.racerId);
    }
    const offered = uniqueOfferedIds.map((id) => findRosterRacer.get(id));
    const requested = uniqueRequestedIds.map((id) => findRosterRacer.get(id));
    if (offered.some((racer) => racer?.team_id !== offeringTeamId)) {
      throw new Error("Every offered racer must be on the offering team.");
    }
    if (requested.some((racer) => racer?.team_id !== receivingTeamId)) {
      throw new Error("Every requested racer must be on the receiving team.");
    }
    assertTeamTradeItemsAvailable(offeringTeamId, offeredItems);
    assertTeamTradeItemsAvailable(receivingTeamId, requestedItems);
    const unavailable = tradeAvailability();
    const offeredUnavailable = new Set(unavailable[offeringTeamId] || []);
    const requestedUnavailable = new Set(unavailable[receivingTeamId] || []);
    if (offeredItems.some((item) => offeredUnavailable.has(tradeItemKey(item)))) {
      throw new Error("One of your offered items is already in a pending trade.");
    }
    if (requestedItems.some((item) => requestedUnavailable.has(tradeItemKey(item)))) {
      throw new Error("One of the requested items is already in a pending trade.");
    }
    const result = insertTradeOffer.run(
      offeringTeamId,
      receivingTeamId,
      uniqueOfferedIds[0] || null,
      uniqueRequestedIds[0] || null,
      JSON.stringify(uniqueOfferedIds),
      JSON.stringify(uniqueRequestedIds),
      JSON.stringify(offeredItems),
      JSON.stringify(requestedItems),
      new Date().toISOString(),
    );
    return hydrateTradeOffers([readTradeOffer.get(result.lastInsertRowid)])[0];
  }
  ...
  function respondToTrade(offerId, action) {
    ...
    if (action === "reject") {
      ...
      return hydrateTradeOffers([readTradeOffer.get(offer.id)])[0];
    }

    const offeredIds = parseTradeRacerIds(offer, "offered");
    const requestedIds = parseTradeRacerIds(offer, "requested");
    const offeredItems = parseTradeItems(offer, "offered");
    const requestedItems = parseTradeItems(offer, "requested");
    const offered = offeredIds.map((id) => findRosterRacer.get(id));
    const requested = requestedIds.map((id) => findRosterRacer.get(id));
    if (offered.some((racer) => racer?.team_id !== offer.offering_team_id)
      || requested.some((racer) => racer?.team_id !== offer.receiving_team_id)) {
    ...
    try {
      ...
      resolveTradeOffer.run("accepted", now, offer.id);
  ...
  function signFreeAgent(teamId, freeAgentId, releasedRacerId) {
    ...
    if (released.source === rookieSource(getActiveSeason())) {
      throw new Error("Rookies drafted this season cannot be released to free agency.");
    }
  ...
  function getDevelopment(week = 1) {
    ...
    return {
      ...
      choicesOpen: developmentChoicesOpenForWeek(week),
      trainingSlotsByTeam: Object.fromEntries(teams.map((team) => [
        team.id,
        trainingRightsForTeam(team.id, week).map((right, index) => ({
          slot: index,
          originalTeamId: right.originalTeamId,
          originalSlot: right.slot,
        })),
      ])),
      options: [
      ...
      carUpgradeSlotsByTeam: Object.fromEntries(teams.map((team) => [
        team.id,
        carUpgradeRightsForTeam(team.id, week).map((right, index) => ({
          slot: index,
          originalTeamId: right.originalTeamId,
          originalSlot: right.slot,
        })),
  ...
  function shuffleCourseNames(names, seed) {
    const random = seededRandom(seed);
  ...
  function ensureSeasonCourseSchedule(season = getActiveSeason()) {
    let schedule = readCourseSchedule.all(season);
    if (schedule.length >= WEEKS_PER_SEASON) return schedule;

    const now = new Date().toISOString();
    for (const raceCourse of readSeasonRaceCourses.all(season)) {
      insertCourseSchedule.run(season, raceCourse.week, raceCourse.course_name, now);
    }

    schedule = readCourseSchedule.all(season);
    if (schedule.length >= WEEKS_PER_SEASON) return schedule;

    const courseNames = COURSES.map((course) => course.name);
    const scheduledNames = new Set(schedule.map((row) => row.course_name));
    const previousSchedule = season > 1 ? readCourseSchedule.all(season - 1) : [];
    const previousNames = new Set(previousSchedule.map((row) => row.course_name));
    const requiredNames = season > 1 && previousSchedule.length
      ? courseNames.filter((name) => !previousNames.has(name))
      : [];
    const candidates = [
      ...requiredNames.filter((name) => !scheduledNames.has(name)),
      ...shuffleCourseNames(
        courseNames.filter((name) => (
          !scheduledNames.has(name) && !requiredNames.includes(name)
        )),
        hashText(`season-course-fill:${season}`),
      ),
    ];
    const weeks = Array.from({ length: WEEKS_PER_SEASON }, (_, index) => index + 1)
      .filter((week) => !schedule.some((row) => row.week === week));
    const namesToSchedule = shuffleCourseNames(
      candidates.slice(0, weeks.length),
      hashText(`season-course-order:${season}`),
    );
    weeks.forEach((week, index) => {
      insertCourseSchedule.run(season, week, namesToSchedule[index], now);
    });
    return readCourseSchedule.all(season);
  }

  function courseNameForSeasonWeek(season, week) {
    const scheduled = ensureSeasonCourseSchedule(season)
      .find((row) => row.week === week);
    return scheduled?.course_name || COURSES[(week - 1) % COURSES.length].name;
  }
  ...
  function openingDraftOrder(season = getActiveSeason()) {
    if (season <= 1) {
      const draft = readDraftState.get();
      const seed = draft?.seed ?? `season-1-opening-draft:${season}`;
      return seededShuffle(teams.map((team) => team.id), `season-1-opening-draft:${seed}`);
    }
  ...
  function getRaceCenter(season = 1) {
    ...
    return {
      ...
      racesRun,
      trackName,
      courseSchedule: courseSchedule.map((row) => ({
        season: row.season,
        week: row.week,
        courseName: row.course_name,
      })),
      ...
      teamTotals: calculateTeamTotals(
        completedAllRaces,
        season,
        seasonChampionships.teams,
        darkSacrificeBonusesForSeason,
      ),
      lapPerformance: {
  ...
  function beginNextSeason() {
    ...
    try {
      ...
      clearDraftRetentionSelections.run(nextSeason);
      clearPitCoachSelections.run(nextSeason);
  ...
  function chooseWeeklyUpgrade(teamId, trainings, week = 1) {
    const now = new Date();
    applyDueDevelopment(now);
    assertDevelopmentSeasonStarted(week);
    assertDevelopmentChoiceOpen(week, now);
    if (!teams.some((team) => team.id === teamId)) throw new Error("Unknown team.");
    const existingChoice = readAppliedTeamUpgradeChoice.get(week, teamId);
    if (existingChoice) {
    ...
    const ownedSlots = trainingRightsForTeam(teamId, week);
    const normalizedTrainings = Array.isArray(trainings)
      ? trainings.slice(0, ownedSlots.length).map((training, slot) => ({
        slot,
        racerId: String(training?.racerId || ""),
        optionIndex: Number(training?.optionIndex),
      }))
      : [];
    if (!ownedSlots.length) {
      throw new Error("This team does not currently own a weekly training.");
    }
    if (normalizedTrainings.length !== ownedSlots.length) {
      throw new Error(`Choose ${ownedSlots.length} weekly trainings.`);
    }
    if (new Set(normalizedTrainings.map((training) => training.racerId)).size !== normalizedTrainings.length) {
      throw new Error("You must select different drivers for each training.");
    }
    const choices = normalizedTrainings.map((training) => {
      if (!Number.isInteger(training.optionIndex) || training.optionIndex < 0 || training.optionIndex > 2) {
        throw new Error("Unknown weekly training option.");
      }
      const racer = findRosterRacer.get(training.racerId);
      if (!racer || racer.team_id !== teamId) {
        throw new Error("That racer is not on this team.");
      }
      if (!readRacerParticipation.get(week, training.racerId)) {
        throw new Error("Only racers who drove in a race this week can receive training.");
      }
      const stat = options[training.optionIndex];
      const growth = readGrowth.get(training.racerId, stat);
      if (!growth) throw new Error("Racer growth data is unavailable.");
      return {
        ...training,
        stat,
        racer,
        result: growth.remaining > 0 ? "improved" : "capped",
      };
    });

    database.exec("BEGIN IMMEDIATE");
    try {
      deletePendingTeamUpgradeChoices.run(week, teamId);
      const createdAt = new Date().toISOString();
      for (const choice of choices) {
    ...
    const appliedChoices = readTeamUpgradeChoices.all(week, teamId);
    return {
      week,
      teamId,
      choices: appliedChoices.map((choice) => ({
        slot: choice.slot,
        racerId: choice.racer_id,
        stat: choice.stat,
        result: choice.result,
        appliedAt: choice.applied_at || null,
        racer: publicRacer(readRoster.all(teamId).find((item) => item.id === choice.racer_id)),
      })),
    };
  }

  function chooseWeeklyCarUpgrade(teamId, carIndex, optionIndex, week = 1, slot = 0) {
    const now = new Date();
    applyDueDevelopment(now);
    assertDevelopmentSeasonStarted(week);
    assertDevelopmentChoiceOpen(week, now);
    if (!teams.some((team) => team.id === teamId)) throw new Error("Unknown team.");
    const existingChoices = readTeamCarUpgradeChoices.all(week, teamId);
    if (existingChoices.some((choice) => choice.applied_at)) {
      throw new Error("This team's weekly car upgrade has already been applied.");
    }
    const ownedSlots = carUpgradeRightsForTeam(teamId, week);
    if (!ownedSlots.length) throw new Error("This team does not currently own a weekly car upgrade.");
    const slotNumber = Number(slot);
    if (!Number.isInteger(slotNumber) || slotNumber < 0 || slotNumber >= ownedSlots.length) {
      throw new Error("Unknown weekly car upgrade slot.");
    }
    ...
    return {
      ...
      carIndex,
      stat,
      appliedAt: readTeamCarUpgradeChoices.all(week, teamId)
        .find((choice) => choice.slot === slotNumber)?.applied_at || null,
  ...
  if (countSeasonRaces.get(startupSeason).count === 0) {
    const firstWeekKey = seasonWeekKey(startupSeason, 1);
    const lastWeekKey = seasonWeekKey(startupSeason, WEEKS_PER_SEASON);
    deletePendingCarUpgradeChoicesForSeason.run(firstWeekKey, lastWeekKey);
  }

  const resetCourseLapStatsMigrationKey = "reset-race-laps-for-six-course-rebalance";
  if (!readAppMigration.get(resetCourseLapStatsMigrationKey)) {
    database.exec("BEGIN IMMEDIATE");
    try {
      deleteRaceLaps.run();
      insertAppMigration.run(resetCourseLapStatsMigrationKey, new Date().toISOString());
      database.exec("COMMIT");
    } catch (error) {
      database.exec("ROLLBACK");
      throw error;
  ...
  return {
    ...
    respondToTrade,
    saveTeamPlan,
    saveOpeningDraftRetention,
    savePitCoachSpecialty,
    chooseWeeklyUpgrade,
