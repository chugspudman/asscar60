...
function statTilesMarkup(racer) {
  ...
  return `
    <div class="ratings" aria-label="${escapeAttribute(racer.name)} ratings">
      ${stats.map(([abbreviation, value, label]) => `
        <span class="rating ${abbreviation === "POT" ? "potential" : ""} ${abbreviation === "STA" && racer.speed_mark ? "marked" : ""} ${racer.cappedStats?.includes(label.toLocaleLowerCase()) ? "capped" : ""}" title="${label}${racer.cappedStats?.includes(label.toLocaleLowerCase()) ? " (discovered cap)" : ""}${abbreviation === "STA" && racer.speed_mark ? " / Mark of The Speed God" : ""}">
          <span class="rating-label">${abbreviation}</span>
          <strong class="rating-value">${value}</strong>
        </span>
      `).join("")}
