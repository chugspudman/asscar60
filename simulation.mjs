...
function performanceTimeMultiplier(pace, speed) {
  const combinedRating = 0.5 * clamp(pace, 1, 10) + 0.5 * clamp(speed, 1, 10);
  return linearScale(combinedRating, 1.27, 1);
}
...
export function simulateRace(entries, seed = "opening-bell", options = {}) {
  ...
  const effectiveStats = (entry) => {
    ...
    return {
      ...
      pace: clamp(driver.pace + (bonuses.pace || 0) - driverFatigue, 1, 10),
      ...
      for (const segmentNumber of crossedSegments) {
        ...
        if (segment.type === "straight") {
          ...
          if (attackerWinsOvertake(
            ...
            tickEndPosition = tickStates.get(entry.id).endPosition;
