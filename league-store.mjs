...
function generateRaceRecap(race) {
  ...
  if (rivalry && rivalry.count >= 2) {
    ...
    paragraphTwoParts.push(recapChoice([
      ...
      `${firstCar} had repeated clashes with ${secondCar}, the two machines giving us a heated show with ${rivalry.count} overtake attempts between them as they jockeyed for position.`,
    ], `${keyPrefix}:rivalry`));
  }
  if (incidentLeader && incidentLeader.count + incidentLeader.spins > 1) {
    if (incidentLeader.spins > 0) {
      paragraphTwoParts.push(`${incidentLeader.carName} was sliding all over the track, collecting ${incidentLeader.count} reported mishaps and ${incidentLeader.spins} spin-out${incidentLeader.spins === 1 ? "" : "s"}.`);
    } else {
      paragraphTwoParts.push(recapChoice([
        ...
        `${incidentLeader.carName} was all over the place, with a reported ${incidentLeader.count} mishaps (${incidentLeader.cornerMishaps} inside turns and chicanes, and ${incidentLeader.overtakeMishaps} from failed overtakes).`,
      ], `${keyPrefix}:mishaps`));
    }
  } else if (incidentLeader && incidentLeader.count + incidentLeader.spins === 1) {
    paragraphTwoParts.push(`${incidentLeader.carName} supplied one of the sharper mishaps, a stark reminder that the Speed God waits for no one.`);
  }
  if (strangeEvents.length) {
    ...
    paragraphTwoParts.push(recapChoice([
      `The weird-shit-o-meter recorded ${strangeEvents.length} strange effect${strangeEvents.length === 1 ? "" : "s"}: ${good} blessed, ${bad} cursed.${strangeDriver ? ` ${strangeDriver} showed a particular affinity for the otherworldly, at times seeming to shimmer and dance across the black top.` : ""}`,
      strangeDriver
        ? `The Stewards (their radiance emanating), have identified ${strangeDriver} among all others as having repeatedly tangoed with the extraplanar forces of speed and drag during this race.`
...
export function createLeagueStore(path = ":memory:") {
  ...
  function buildLapPerformanceSummaries() {
    ...
    const courseRoleAverages = {};
    for (const course of COURSES) {
      courseRoleAverages[course.name] = {};
      for (const role of Object.keys(ROLE_CODES)) {
        courseRoleAverages[course.name][role] = formatPerformanceValue(
          average(leagueCourseRole.get(`${course.name}:${role}`) || []),
        );
      }
    }
    ...
    for (const [racerId, racerLapRows] of racerRows) {
      ...
      for (const course of COURSES) {
        for (const role of Object.keys(ROLE_CODES)) {
          ...
          const leagueAverage = courseRoleAverages[course.name][role];
          vsCourseRole[code] = racerAverage === null || leagueAverage === null
            ? null
            : formatPerformanceValue(racerAverage - leagueAverage);
    ...
    return { byRacer, courseRoleAverages };
  }
  ...
  function getRaceCenter(season = 1) {
    ...
    return {
      ...
      lapPerformance: {
        courseRoleAverages: lapPerformance.courseRoleAverages,
      },
  ...
  function finishRace(raceId) {
    ...
    try {
      ...
      updateRaceRecap.run(generateRaceRecap(race), race.id);
