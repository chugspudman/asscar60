...
const PLACEHOLDER_PIT_COACH_OPTIONS = Array.from({ length: 12 }, (_, index) => ({
...
const elements = {
  ...
  saveDraftRetention: document.querySelector("#save-draft-retention"),
  ...
  addTradeOfferItem: document.querySelector("#add-trade-offer-item"),
  tradeRecipient: document.querySelector("#trade-recipient"),
  tradeTeam: document.querySelector("#trade-team"),
  tradeRequestItems: document.querySelector("#trade-request-items"),
  addTradeRequestItem: document.querySelector("#add-trade-request-item"),
...
function renderRaceProgress(standings = [], label = "") {
  ...
  const featureMarkers = course.segments.map((segment, index) => {
    ...
    const code = segment.type === "turn" ? "T" : "C";
    return `
...
function renderCourses() {
  const scheduleByCourse = Object.fromEntries(
    (state.raceCenter.courseSchedule || []).map((item) => [item.courseName, item.week]),
  );
  elements.courseList.innerHTML = COURSES.map((course, index) => {
    ...
    const scheduledWeek = scheduleByCourse[course.name] || null;
    return `
      <article class="course-card ${state.raceCenter.week === scheduledWeek ? "current" : ""}">
        <div class="course-card-heading">
          <span class="course-week">${scheduledWeek ? `Week ${scheduledWeek}` : "Off this season"}</span>
          <h3>${escapeHtml(course.name)}</h3>
          <span class="course-length">Length ${course.length} / 6</span>
        </div>
...
function racerIdentityMarkup(racer) {
  return `
    <div class="racer-identity">
      ...
      ${racer.speed_mark ? `
        <span
          class="speed-mark-icon"
          role="img"
          aria-label="Mark of The Speed God"
          title="Mark of The Speed God"
        ></span>
      ` : ""}
...
function renderLeague() {
  ...
  elements.teamGrid.innerHTML = teams.map((team) => {
    ...
    const totals = state.raceCenter.teamTotals?.[team.id] || {};
    const points = totals.seasonPoints ?? pointsByTeam.get(team.id) ?? 0;
    const franchisePoints = totals.franchisePoints ?? points;
    ...
    return `
      <article class="league-card" style="--team-color:${team.color}">
        ...
        <div class="league-team-record">
          <span><strong>${pointsLabel(points)}</strong> Season points</span>
          <span><strong>${pointsLabel(franchisePoints)}</strong> Franchise points</span>
          <span><strong>${titles}</strong> Championship wins</span>
          <span><strong>${totals.goldMedals || 0}</strong> Gold medals</span>
          <span><strong>${totals.silverMedals || 0}</strong> Silver medals</span>
          <span><strong>${totals.bronzeMedals || 0}</strong> Bronze medals</span>
        </div>
...
function racerDirectoryCard(racer, { showAffiliation = true } = {}) {
  ...
  const previousTeams = (racer.previousTeamIds || [])
    .map((teamId) => state.brands[teamId]?.abbreviation || teams.find((item) => item.id === teamId)?.short || teamId)
    .filter(Boolean);
  const previousTeamsLabel = previousTeams.length ? previousTeams.join(", ") : "None";
    ...
    <article class="directory-racer" data-racer-card="${racer.id}">
      ...
      <div class="racer-expanded" hidden>
        <div class="racer-bio">
          ...
          <dl>
            <div><dt>Current Team</dt><dd>${escapeHtml(affiliation)}</dd></div>
            <div><dt>League origin</dt><dd>${escapeHtml(racer.leagueOrigin)}</dd></div>
            <div><dt>Potential remaining</dt><dd>${racer.potential}</dd></div>
            <div><dt>Previous Teams</dt><dd>${escapeHtml(previousTeamsLabel)}</dd></div>
          </dl>
        </div>
  ...
  elements.memoriamList.innerHTML = state.inMemoriam.length
    ? state.inMemoriam.map((entry) => `
      <article class="memoriam-row">
        <div class="memoriam-driver">
          <strong>${escapeHtml(entry.name)}</strong>
          <small>${Number(entry.careerLaps || 0)} career laps / ${Number(entry.careerPodiums || 0)} career podiums</small>
        </div>
...
function statTilesMarkup(racer) {
  ...
  return `
    <div class="ratings" aria-label="${escapeAttribute(racer.name)} ratings">
      ${stats.map(([abbreviation, value, label]) => `
        <span class="rating ${abbreviation === "POT" ? "potential" : ""} ${racer.cappedStats?.includes(label.toLocaleLowerCase()) ? "capped" : ""}" title="${label}${racer.cappedStats?.includes(label.toLocaleLowerCase()) ? " (discovered cap)" : ""}">
          <span class="rating-label">${abbreviation}</span>
          <strong class="rating-value">${value}</strong>
        </span>
      `).join("")}
...
function renderDraftInitiation() {
  ...
  const required = initiation.requiredVotes || teams.length;
  const retentionCount = initiation.retentionRequired ? initiation.retentionCount || 0 : required;
  const coachCount = initiation.coachRequired ? initiation.coachCount || 0 : required;
  const voteLine = initiation.allTeamsAssigned
    ? `${initiation.voteCount} of ${required} teams have voted to begin the opening draft.`
    : "Waiting for all teams to be assigned before the season can be initiated.";
  const retentionLine = initiation.retentionRequired
    ? `${retentionCount} of ${required} teams have selected their retainers.`
    : "Retainers are not required before the first opening draft.";
  const coachLine = initiation.coachRequired
    ? `${coachCount} of ${required} teams have selected their Pit Coach specialty.`
    : `${required} of ${required} teams have selected their Pit Coach specialty.`;
  if (!initiation.allTeamsAssigned) {
    elements.draftInitiationProgress.innerHTML = `<p>${voteLine}</p>`;
  } else {
    elements.draftInitiationProgress.innerHTML = `
      <p>${escapeHtml(retentionLine)}</p>
      <p>${escapeHtml(coachLine)}</p>
      <p>${escapeHtml(voteLine)}</p>
    `;
  }
...
function retentionStateForManagedTeam() {
  const initiation = state.draft?.initiation || {};
  ...
  return {
    ...
    retention,
    selected,
    required: state.draft.status === "not_started"
      && initiation.retentionRequired
      && initiation.allTeamsAssigned,
    complete: Boolean(selected.veteranRacerId && selected.rookieRacerId),
  };
}

function needsRetentionSelection() {
  const retention = retentionStateForManagedTeam();
  return retention.required && !retention.complete;
}

function pitCoachStateForManagedTeam() {
  const initiation = state.draft?.initiation || {};
  const options = initiation.pitCoachOptions?.length
    ? initiation.pitCoachOptions
    : PLACEHOLDER_PIT_COACH_OPTIONS;
  return {
    initiation,
    options,
    selected: initiation.pitCoachSelection || null,
    required: state.draft.status === "not_started"
      && initiation.coachRequired
      && initiation.allTeamsAssigned,
    complete: Boolean(initiation.pitCoachSelection?.specialtyKey),
  };
}

function needsPitCoachSelection() {
  const coach = pitCoachStateForManagedTeam();
...
function renderAdminHeader() {
  ...
  elements.movesPitCoachHeader.innerHTML = `
    <div><span>Pit Coach and Specialty:</span> <strong>${escapeHtml(selected.coachName)} // ${escapeHtml(specialty?.name || selected.specialtyKey || "Specialty")}</strong></div>
...
function renderAdminRetention() {
  ...
  const readyToSave = hasOptions && elements.draftRetainVeteran.value && elements.draftRetainRookie.value;
  elements.saveDraftRetention.disabled = !readyToSave;
  elements.saveDraftRetention.textContent = complete ? "Update retainers" : "Save retainers";
  if (elements.draftRetentionMessage && !elements.draftRetentionMessage.textContent) {
    elements.draftRetentionMessage.textContent = hasOptions
      ? "The draft begins when every team saves retainers, saves a Pit Coach specialty, and votes."
      : "Your roster does not have eligible veteran and rookie retainers.";
  }
  updateDraftVoteAlertBadges();
}
...
function optionMarkup(items, selectedId = null) {
  const selectedIds = new Set(Array.isArray(selectedId) ? selectedId : [selectedId].filter(Boolean));
  return items.map((item) => `
    <option value="${item.id}" ${selectedIds.has(item.id) ? "selected" : ""}>${escapeHtml(item.name)}</option>
  `).join("");
}

function tradeRacerNames(offer, side) {
  const racers = side === "offered" ? offer.offeredRacers : offer.requestedRacers;
  if (Array.isArray(racers) && racers.length) return racers.map((racer) => racer.name).join(", ");
  return side === "offered" ? offer.offered_racer_name : offer.requested_racer_name;
}

function tradeItemLabels(offer, side) {
  const labels = side === "offered" ? offer.offeredItemLabels : offer.requestedItemLabels;
  return Array.isArray(labels) && labels.length
    ? labels.join("; ")
    : tradeRacerNames(offer, side);
}
...
function unavailableTradeItemKeys(teamId) {
  return new Set(state.transactions.unavailableTradeItemsByTeam?.[teamId] || []);
}

function tradableTradeItemKeys(teamId) {
  return new Set(state.transactions.tradableTradeItemsByTeam?.[teamId] || []);
}

function currentTradeRecipientTeam() {
  const actingTeamId = managedTeamId();
  const otherTeams = teams.filter((team) => team.id !== actingTeamId);
  return otherTeams.find((team) => team.id === state.tradeBuilder.recipientTeamId) || null;
}

function availableTradeDrivers(teamId, items) {
  const team = teams.find((item) => item.id === teamId);
  const unavailable = unavailableTradeItemKeys(teamId);
  const used = new Set(items.filter((item) => item.type === "driver" && item.racerId).map((item) => item.racerId));
  return (team?.drivers || []).filter((racer) => (
    !unavailable.has(`driver:${racer.id}`) && !used.has(racer.id)
  ));
}

function canAddTradeItem(teamId, item, items) {
  if (!teamId) return false;
  if (items.length >= MAX_TRADE_ITEMS_PER_SIDE) return false;
  if (item.type === "driver") return availableTradeDrivers(teamId, items).length > 0;
  const unavailable = unavailableTradeItemKeys(teamId);
  const tradable = tradableTradeItemKeys(teamId);
  const used = new Set(items.map(tradeItemKey));
  if (item.type === "training") {
    return tradable.has(`training:${item.amount}`)
      && !unavailable.has("training:both")
      && !(item.amount === "both" && unavailable.has("training:single"))
      && !used.has("training:both")
      && !(item.amount === "both" && used.has("training:single"))
      && !used.has(`training:${item.amount}`)
      && !unavailable.has(`training:${item.amount}`);
  }
  return tradable.has(tradeItemKey(item))
    && !unavailable.has(tradeItemKey(item))
    && !used.has(tradeItemKey(item));
}

function tradeItemTypeOptions(teamId, items) {
  const typeOptions = [
    ["driver", "Driver", canAddTradeItem(teamId, { type: "driver" }, items)],
    ["training", "Next Training", canAddTradeItem(teamId, { type: "training", amount: "single" }, items)
      || canAddTradeItem(teamId, { type: "training", amount: "both" }, items)],
    ["upgrade", "Next Upgrade", canAddTradeItem(teamId, { type: "upgrade" }, items)],
    ["draft_pick", "Opening Draft Pick", [1, 2, 3, 4, 5, 6].some((round) => canAddTradeItem(teamId, { type: "draft_pick", round }, items))],
  ];
  return typeOptions.map(([value, label, enabled]) => `
    <option value="${value}" ${enabled ? "" : "disabled"}>${label}</option>
  `).join("");
}

function renderTradeItemRow(side, item, teamId, items) {
  const sideLabel = side === "offer" ? "Offer" : "Request";
  const siblingItems = items.filter((entry) => entry.id !== item.id);
  if (!item.type) {
    return `
      <div class="trade-builder-row pending" data-trade-side="${side}" data-trade-item-id="${item.id}">
        <select data-trade-type>
          <option value="">Choose item type</option>
          ${tradeItemTypeOptions(teamId, siblingItems)}
        </select>
        <button type="button" class="trade-remove-item" data-remove-trade-item aria-label="Remove ${sideLabel} item">x</button>
      </div>
    `;
  }
  if (item.type === "driver" && !item.racerId) {
    const drivers = availableTradeDrivers(teamId, siblingItems);
    return `
      <div class="trade-builder-row pending" data-trade-side="${side}" data-trade-item-id="${item.id}">
        <span>Driver:</span>
        <select data-trade-driver>
          <option value="">Select driver</option>
          ${drivers.map((racer) => `<option value="${racer.id}">${escapeHtml(racer.name)}</option>`).join("")}
        </select>
        <button type="button" class="trade-remove-item" data-remove-trade-item aria-label="Remove ${sideLabel} item">x</button>
      </div>
    `;
  }
  if (item.type === "training" && !item.amount) {
    return `
      <div class="trade-builder-row pending" data-trade-side="${side}" data-trade-item-id="${item.id}">
        <span>Next training:</span>
        <select data-trade-training>
          <option value="">Select amount</option>
          <option value="single" ${canAddTradeItem(teamId, { type: "training", amount: "single" }, siblingItems) ? "" : "disabled"}>Single</option>
          <option value="both" ${canAddTradeItem(teamId, { type: "training", amount: "both" }, siblingItems) ? "" : "disabled"}>Both</option>
        </select>
        <button type="button" class="trade-remove-item" data-remove-trade-item aria-label="Remove ${sideLabel} item">x</button>
      </div>
    `;
  }
  if (item.type === "draft_pick" && !item.round) {
    return `
      <div class="trade-builder-row pending" data-trade-side="${side}" data-trade-item-id="${item.id}">
        <span>Opening Draft Pick:</span>
        <select data-trade-pick>
          <option value="">Select round</option>
          ${[1, 2, 3, 4, 5, 6].map((round) => `
            <option value="${round}" ${canAddTradeItem(teamId, { type: "draft_pick", round }, siblingItems) ? "" : "disabled"}>${ordinal(round)} round</option>
          `).join("")}
        </select>
        <button type="button" class="trade-remove-item" data-remove-trade-item aria-label="Remove ${sideLabel} item">x</button>
      </div>
  ...
  const driver = team?.drivers?.find((racer) => racer.id === item.racerId);
  const value = item.type === "driver"
    ? driver?.name || item.racerId
    : item.type === "training"
      ? (item.amount === "both" ? "Both" : "Single")
      : item.type === "upgrade"
        ? "Next Upgrade"
        : `${ordinal(item.round)} round`;
  const label = item.type === "driver"
    ...
    : item.type === "training"
      ...
      : item.type === "upgrade"
        ...
        : "Opening Draft Pick:";
  return `
    <div class="trade-builder-row complete" data-trade-side="${side}" data-trade-item-id="${item.id}">
      <span>${label || " "}</span>
...
function renderTradeItemList(side) {
  const items = side === "offer" ? state.tradeBuilder.offerItems : state.tradeBuilder.requestItems;
  const teamId = side === "offer" ? managedTeamId() : currentTradeRecipientTeam()?.id;
  const container = side === "offer" ? elements.tradeOfferItems : elements.tradeRequestItems;
  const addButton = side === "offer" ? elements.addTradeOfferItem : elements.addTradeRequestItem;
  if (!container || !addButton) return;
  container.innerHTML = items.length
    ? items.map((item) => renderTradeItemRow(side, item, teamId, items)).join("")
    : `<p class="muted">No ${side === "offer" ? "offer" : "request"} items yet.</p>`;
  addButton.hidden = items.length >= MAX_TRADE_ITEMS_PER_SIDE || (side === "request" && !teamId);
  addButton.disabled = items.length >= MAX_TRADE_ITEMS_PER_SIDE || (side === "request" && !teamId);
}

function renderTradeRecipient() {
  const actingTeamId = managedTeamId();
  const otherTeams = teams.filter((team) => team.id !== actingTeamId);
  const recipient = currentTradeRecipientTeam();
  if (!elements.tradeRecipient) return;
  elements.tradeRecipient.innerHTML = `
    <h3>To</h3>
    ${recipient && state.tradeBuilder.recipientLocked ? `
      <div class="trade-builder-row complete">
        <strong>${escapeHtml(recipient.short)} / ${escapeHtml(recipient.name)}</strong>
        <button type="button" class="trade-remove-item" data-clear-trade-recipient aria-label="Clear trade recipient">x</button>
      </div>
    ` : `
      <label class="trade-builder-row pending">
        <span>Team:</span>
        <select id="trade-team">
          <option value="">Select team</option>
          ${otherTeams.map((team) => `
            <option value="${team.id}" ${team.id === state.tradeBuilder.recipientTeamId ? "selected" : ""}>${team.short} / ${team.name}</option>
          `).join("")}
        </select>
      </label>
    `}
  `;
  elements.tradeTeam = elements.tradeRecipient.querySelector("#trade-team");
}

function addTradeBuilderItem(side) {
  const items = side === "offer" ? state.tradeBuilder.offerItems : state.tradeBuilder.requestItems;
  if (items.length >= MAX_TRADE_ITEMS_PER_SIDE) return;
  items.push({ id: state.tradeBuilder.nextId++ });
  renderTradeBuilder();
}

function removeTradeBuilderItem(side, id) {
  const key = side === "offer" ? "offerItems" : "requestItems";
  state.tradeBuilder[key] = state.tradeBuilder[key].filter((item) => item.id !== id);
  renderTradeBuilder();
}

function tradeItemsReady(items) {
  return items.length > 0 && items.every((item) => (
    item.type === "upgrade"
    || (item.type === "driver" && item.racerId)
    || (item.type === "training" && item.amount)
    || (item.type === "draft_pick" && item.round)
  ));
}

function renderTradeBuilder() {
  renderTradeRecipient();
  renderTradeItemList("offer");
  renderTradeItemList("request");
  const ready = tradeItemsReady(state.tradeBuilder.offerItems)
    && state.tradeBuilder.recipientTeamId
    && tradeItemsReady(state.tradeBuilder.requestItems);
  elements.proposeTrade.disabled = !ready;
}
    ...
    ? visibleOffers.map((offer) => {
      ...
      return `
        <article class="trade-offer">
          <strong>${offering.short} offers ${escapeHtml(tradeItemLabels(offer, "offered"))}</strong>
          <p>To ${receiving.name} for ${escapeHtml(tradeItemLabels(offer, "requested"))}.</p>
          ${canRespond ? `
  ...
  elements.transactionHistory.innerHTML = state.transactions.history.length
    ? state.transactions.history.map((transaction) => {
      ...
      const text = transaction.type === "trade"
        ? `${team.short} ${transaction.acquired_racer_name ? `acquired ${transaction.acquired_racer_name}` : "completed a trade"} in a trade.`
...
function updateTrainingLockEligibility() {
  const button = elements.upgradeOptions.querySelector("[data-lock-trainings]");
  const error = elements.upgradeOptions.querySelector("[data-training-error]");
  const racerSelects = [...elements.upgradeOptions.querySelectorAll("[data-training-racer]")];
  if (!button || !error || !racerSelects.length || button.disabled && !racerSelects.every((select) => !select.disabled)) return;
  const selected = racerSelects.map((select) => select.value).filter(Boolean);
  const duplicateDriver = new Set(selected).size !== selected.length;
  error.textContent = duplicateDriver ? "You must select different drivers for each training." : "";
  button.disabled = duplicateDriver;
}

function renderDevelopment() {
  const actingTeamId = managedTeamId();
  const actingTeam = teams.find((team) => team.id === actingTeamId);
  const teamChoices = state.development.choices
    .filter((choice) => choice.team_id === actingTeamId)
    .sort((a, b) => (a.slot ?? 0) - (b.slot ?? 0));
  const carChoices = state.development.carChoices
    .filter((choice) => choice.team_id === actingTeamId)
    .sort((a, b) => (a.slot ?? 0) - (b.slot ?? 0));
  ...
  const trainingSlots = state.development.trainingSlotsByTeam?.[actingTeamId] || [{ slot: 0 }, { slot: 1 }];
  const carUpgradeSlots = state.development.carUpgradeSlotsByTeam?.[actingTeamId] || [{ slot: 0 }];
  const developmentChoicesOpen = state.development.choicesOpen === true;
  const trainingApplied = teamChoices.some((choice) => choice.applied_at);
  const trainingUnavailable = !developmentChoicesOpen
    || trainingApplied
    || !trainingSlots.length
    || eligibleRacers.length < trainingSlots.length;

  elements.developmentTeam.textContent = managedTeamLabel();

  const statOptions = (selectedIndex = 0) => state.development.options.map((stat, optionIndex) => `
    <option value="${optionIndex}" ${Number(selectedIndex) === optionIndex ? "selected" : ""}>${titleCase(stat)}</option>
  `).join("");
  const racerOptions = (selectedId = "") => eligibleRacers.length
    ? eligibleRacers.map((racer) => `
      <option value="${racer.id}" ${selectedId === racer.id ? "selected" : ""}>${escapeHtml(racer.name)}</option>
    `).join("")
    : `<option>No racers have driven this week</option>`;
  const choiceForSlot = (slot) => teamChoices.find((choice) => Number(choice.slot || 0) === slot);
  const chosenDefaults = new Set();
  const trainingRows = trainingSlots.map((slotInfo, index) => {
    const choice = choiceForSlot(index) || teamChoices[index] || null;
    const defaultRacer = choice?.racer_id
      || eligibleRacers.find((racer) => !chosenDefaults.has(racer.id))?.id
      || eligibleRacers[index]?.id
      || "";
    if (defaultRacer) chosenDefaults.add(defaultRacer);
    return `
      <p class="development-choice-line">
        Increase the
        <select data-training-stat="${index}" aria-label="Weekly training ${index + 1} trait" ${trainingUnavailable ? "disabled" : ""}>
          ${statOptions(choice?.option_index ?? 0)}
        </select>
        of
        <select data-training-racer="${index}" aria-label="Weekly training ${index + 1} driver" ${trainingUnavailable ? "disabled" : ""}>
          ${racerOptions(defaultRacer)}
        </select>
        by 1.
      </p>`;
  }).join("");
  elements.upgradeOptions.innerHTML = `
    <article class="development-choice-card ${trainingUnavailable ? "unavailable" : ""} ${developmentChoicesOpen && teamChoices.length && !trainingApplied ? "selected" : ""}">
      ${trainingRows || `<p class="muted">This team does not currently own a weekly training.</p>`}
      <p class="development-choice-error" data-training-error></p>
      <button class="primary-button" data-lock-trainings ${trainingUnavailable ? "disabled" : ""}>
        ${trainingApplied ? "Trainings applied" : teamChoices.length ? "Update lock-in" : "Lock In"}
      </button>
      <p class="muted">${trainingApplied
        ? "This week's trainings have already been applied."
        : !developmentChoicesOpen
          ? "Training and Upgrade choices open once the first race of the season has begun."
        : !trainingSlots.length
          ? "This team has traded away its weekly trainings."
          : eligibleRacers.length < trainingSlots.length
            ? `${trainingSlots.length} different racers must drive this week before trainings can be locked in.`
            : "The selections lock in now and apply Sunday at 12:01 AM Eastern. You can change them until then."}</p>
    </article>`;
  updateTrainingLockEligibility();

  const carApplied = carChoices.some((choice) => choice.applied_at);
  const carUpgradeUnavailable = !developmentChoicesOpen || carApplied || !carUpgradeSlots.length;
  const carChoiceForSlot = (slot) => carChoices.find((choice) => Number(choice.slot || 0) === slot);
  const carUpgradeRows = carUpgradeSlots.map((slotInfo, index) => {
    const choice = carChoiceForSlot(index) || carChoices[index] || null;
    return `
      <p class="development-choice-line">
        Increase the
        <select data-car-training-stat="${index}" aria-label="Weekly car upgrade ${index + 1} trait" ${carUpgradeUnavailable ? "disabled" : ""}>
          ${state.development.carOptions.map((stat, optionIndex) => `
            <option value="${optionIndex}" ${choice?.option_index === optionIndex ? "selected" : ""}>${titleCase(stat)}</option>
          `).join("")}
        </select>
        of
        <select data-upgrade-car="${index}" aria-label="Weekly upgrade ${index + 1} car" ${carUpgradeUnavailable ? "disabled" : ""}>
          ${state.carNames[actingTeamId].map((name, carIndex) => `
            <option value="${carIndex}" ${choice?.car_index === carIndex ? "selected" : ""}>${actingTeam.short} ${escapeHtml(name)}</option>
          `).join("")}
        </select>
        by 1.
      </p>`;
  }).join("");
  elements.carUpgradeOptions.innerHTML = `
    <article class="development-choice-card ${carUpgradeUnavailable ? "unavailable" : ""} ${developmentChoicesOpen && carChoices.length && !carApplied ? "selected" : ""}">
      ${carUpgradeRows || `<p class="muted">This team does not currently own a weekly car upgrade.</p>`}
      <button class="primary-button" data-car-upgrade-option ${carUpgradeUnavailable ? "disabled" : ""}>
        ${carApplied ? "Upgrade applied" : carChoices.length ? "Update lock-in" : "Lock In"}
      </button>
      <p class="muted">${carApplied
        ? "This week's car upgrade has already been applied."
        : !developmentChoicesOpen
          ? "Training and Upgrade choices open once the first race of the season has begun."
        : !carUpgradeSlots.length
          ? "This team has traded away its weekly car upgrade."
          : "The upgrade locks in now and applies Sunday at 12:01 AM Eastern. You can change it until then."}</p>
    </article>`;
...
function currentAdminRequirementAlertKey() {
  if (!needsRetentionSelection() && !needsPitCoachSelection()) return "";
  const season = state.draft?.initiation?.season || state.raceCenter?.season || 1;
  return `opening-admin:${season}`;
}
...
function markAdminRequirementOfficeSeen() {
  const key = currentAdminRequirementAlertKey();
  if (!key) return;
  state.noticedAdminRequirementAlertKeys = [
    ...new Set([...state.noticedAdminRequirementAlertKeys, key]),
  ];
  updateOfficeAlertBadges();
}
...
function updateOfficeAlertBadges() {
  const officeHasDraftAlert = hasOfficeDraftVoteAlert();
  const draftSubnavHasAlert = hasDraftSubnavVoteAlert();
  const retentionAlert = needsRetentionSelection();
  const coachAlert = needsPitCoachSelection();
  const adminAlert = hasUnreadTradeOffers() || retentionAlert || coachAlert;
  const officeButton = document.querySelector('[data-section="office"]');
  const shouldPointToOffice = hasUnreadTradeOffers()
    || officeHasDraftAlert
    || hasOfficeAdminRequirementAlert();
  officeButton?.classList.toggle("has-alert", shouldPointToOffice);
  document.querySelector('[data-subview="draft"]')?.classList.toggle("has-alert", draftSubnavHasAlert);
  document.querySelector('[data-subview="moves"]')?.classList.toggle("has-alert", adminAlert);
}
...
function activateSection(sectionName, requestedView = null) {
  ...
  elements.subnav.innerHTML = tabs.map((tab) => `
    <button class="subnav-link ${tab.view === viewName ? "active" : ""} ${tab.view === "moves" && (hasUnreadTradeOffers() || needsRetentionSelection() || needsPitCoachSelection()) ? "has-alert" : ""} ${tab.view === "draft" && hasDraftSubnavVoteAlert() ? "has-alert" : ""}" data-subview="${tab.view}">
      ${tab.label}
    </button>
  `).join("");
...
document.querySelectorAll(".nav-link").forEach((button) => {
  button.addEventListener("click", async () => {
    closeAppMenu();
    if (button.dataset.section === "office") {
      markDraftVoteAlertOfficeSeen();
      markAdminRequirementOfficeSeen();
    }
...
function updateRetentionSaveEligibility() {
  if (!elements.saveDraftRetention || elements.draftRetentionFields?.hidden) return;
  elements.saveDraftRetention.disabled = !(
    elements.draftRetainVeteran?.value
    && elements.draftRetainRookie?.value
  );
  if (elements.draftRetentionMessage) {
    elements.draftRetentionMessage.textContent = "";
  }
}

elements.draftRetainVeteran?.addEventListener("change", updateRetentionSaveEligibility);
elements.draftRetainRookie?.addEventListener("change", updateRetentionSaveEligibility);

elements.saveDraftRetention?.addEventListener("click", async () => {
  elements.saveDraftRetention.disabled = true;
  elements.draftRetentionMessage.textContent = "Saving retainers...";
  try {
    const response = await fetch("/api/draft/retention", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        poolSize: 60,
        veteranRetentionId: elements.draftRetainVeteran.value,
        rookieRetentionId: elements.draftRetainRookie.value,
      }),
    });
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || "Could not save retainers.");
    state.draft = result;
    elements.draftRetentionMessage.textContent = result.status === "active"
      ? "All retainers are saved. The opening draft has begun."
      : "Retainers saved.";
    renderDraft();
    renderTransactions();
    renderRaceControls();
  } catch (error) {
    elements.draftRetentionMessage.textContent = error.message;
  } finally {
    renderAdminRetention();
  }
});
...
elements.savePitCoach?.addEventListener("click", async () => {
  elements.savePitCoach.disabled = true;
  elements.pitCoachMessage.textContent = "Saving Pit Coach specialty...";
  try {
    const response = await fetch("/api/draft/pit-coach", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        poolSize: 60,
        specialtyKey: elements.pitCoachSpecialty.value,
      }),
    });
    const result = await response.json();
    if (!response.ok) throw new Error(result.error || "Could not save Pit Coach specialty.");
    state.draft = result;
    elements.pitCoachMessage.textContent = result.status === "active"
      ? "All requirements are complete. The opening draft has begun."
      : "Pit Coach specialty saved.";
    renderDraft();
    renderTransactions();
    renderRaceControls();
  } catch (error) {
    elements.pitCoachMessage.textContent = error.message;
  } finally {
    renderPitCoachSelection();
  }
});
...
elements.addTradeOfferItem?.addEventListener("click", () => addTradeBuilderItem("offer"));
elements.addTradeRequestItem?.addEventListener("click", () => addTradeBuilderItem("request"));

function tradeBuilderItemFromEvent(event) {
  const row = event.target.closest("[data-trade-item-id]");
  if (!row) return {};
  const side = row.dataset.tradeSide;
  const id = Number(row.dataset.tradeItemId);
  const items = side === "offer" ? state.tradeBuilder.offerItems : state.tradeBuilder.requestItems;
  return { row, side, id, item: items.find((entry) => entry.id === id) };
}

elements.tradeRecipient?.addEventListener("change", (event) => {
  if (!event.target.matches("#trade-team")) return;
  state.tradeBuilder.recipientTeamId = event.target.value || null;
  state.tradeBuilder.recipientLocked = Boolean(event.target.value);
  state.tradeBuilder.requestItems = [];
  renderTradeBuilder();
});

elements.tradeRecipient?.addEventListener("click", (event) => {
  if (!event.target.closest("[data-clear-trade-recipient]")) return;
  state.tradeBuilder.recipientTeamId = null;
  state.tradeBuilder.recipientLocked = false;
  state.tradeBuilder.requestItems = [];
  renderTradeBuilder();
});

function updateTradeBuilderItem(event) {
  const { side, item } = tradeBuilderItemFromEvent(event);
  if (!item) return;
  if (event.target.matches("[data-trade-type]")) {
  ...
  if (event.target.matches("[data-trade-driver]")) {
    item.racerId = event.target.value || "";
    if (item.racerId) item.complete = true;
    renderTradeBuilder();
    return;
  }
  if (event.target.matches("[data-trade-training]")) {
    item.amount = event.target.value || "";
    if (item.amount) item.complete = true;
    renderTradeBuilder();
    return;
  }
  if (event.target.matches("[data-trade-pick]")) {
    item.round = Number(event.target.value) || null;
    if (item.round) item.complete = true;
    renderTradeBuilder();
  }
}

elements.tradeOfferItems?.addEventListener("change", updateTradeBuilderItem);
elements.tradeRequestItems?.addEventListener("change", updateTradeBuilderItem);
elements.tradeOfferItems?.addEventListener("click", (event) => {
  if (!event.target.closest("[data-remove-trade-item]")) return;
  const { side, id } = tradeBuilderItemFromEvent(event);
  removeTradeBuilderItem(side, id);
});
elements.tradeRequestItems?.addEventListener("click", (event) => {
  if (!event.target.closest("[data-remove-trade-item]")) return;
  const { side, id } = tradeBuilderItemFromEvent(event);
  removeTradeBuilderItem(side, id);
});
...
elements.proposeTrade.addEventListener("click", async () => {
  const offeredItems = state.tradeBuilder.offerItems;
  const requestedItems = state.tradeBuilder.requestItems;
  const offeredRacerIds = offeredItems.filter((item) => item.type === "driver").map((item) => item.racerId);
  const requestedRacerIds = requestedItems.filter((item) => item.type === "driver").map((item) => item.racerId);
  if (!tradeItemsReady(offeredItems) || !state.tradeBuilder.recipientTeamId || !tradeItemsReady(requestedItems)) {
    elements.movesMessage.textContent = "Complete the offer, recipient, and request before sending.";
    return;
  }
  ...
  try {
    const response = await fetch("/api/trades", {
      ...
      body: JSON.stringify({
        ...
        receivingTeamId: state.tradeBuilder.recipientTeamId,
        offeredRacerIds,
        requestedRacerIds,
        offeredItems,
        requestedItems,
...
elements.upgradeOptions.addEventListener("change", (event) => {
  if (event.target.matches("[data-training-racer]")) updateTrainingLockEligibility();
});

elements.upgradeOptions.addEventListener("click", async (event) => {
  const button = event.target.closest("[data-lock-trainings]");
  if (!button) return;
  updateTrainingLockEligibility();
  if (button.disabled) return;
  const statSelects = [...elements.upgradeOptions.querySelectorAll("[data-training-stat]")]
    .sort((a, b) => Number(a.dataset.trainingStat) - Number(b.dataset.trainingStat));
  const racerSelects = [...elements.upgradeOptions.querySelectorAll("[data-training-racer]")]
    .sort((a, b) => Number(a.dataset.trainingRacer) - Number(b.dataset.trainingRacer));
  button.disabled = true;
  ...
  try {
    const response = await fetch("/api/development/choose", {
      ...
      body: JSON.stringify({
        teamId: managedTeamId(),
        trainings: racerSelects.map((select, slot) => ({
          racerId: select.value,
          optionIndex: Number(statSelects[slot]?.value),
        })),
      }),
    });
    const result = await response.json();
    ...
    renderDevelopment();
    renderBrand();
    const summary = (result.choices || []).map((choice) => (
      choice.appliedAt
        ? choice.result === "improved"
          ? `${choice.racer.name} improved ${titleCase(choice.stat)}`
          : `${choice.racer.name} discovered their ${titleCase(choice.stat)} cap`
        : `${choice.racer.name} for ${titleCase(choice.stat)}`
    )).join("; ");
    elements.developmentMessage.textContent = (result.choices || []).some((choice) => choice.appliedAt)
      ? `${summary}.`
      : `Trainings locked in: ${summary}. They will take effect Sunday at 12:01 AM Eastern.`;
  } catch (error) {
    elements.developmentMessage.textContent = error.message;
    button.disabled = false;
    updateTrainingLockEligibility();
...
elements.carUpgradeOptions.addEventListener("click", async (event) => {
  ...
  if (!button) return;
  if (button.disabled) return;
  const statSelects = [...elements.carUpgradeOptions.querySelectorAll("[data-car-training-stat]")]
    .sort((a, b) => Number(a.dataset.carTrainingStat) - Number(b.dataset.carTrainingStat));
  const carSelects = [...elements.carUpgradeOptions.querySelectorAll("[data-upgrade-car]")]
    .sort((a, b) => Number(a.dataset.upgradeCar) - Number(b.dataset.upgradeCar));
  button.disabled = true;
  elements.developmentMessage.textContent = "Installing weekly car upgrade...";
  try {
    const results = [];
    for (const [slot, carSelect] of carSelects.entries()) {
    ...
    const summary = results.map((result) => {
      ...
      return result.appliedAt
        ...
        : `${carName} for ${titleCase(result.stat)}`;
    }).join("; ");
    elements.developmentMessage.textContent = results.some((result) => result.appliedAt)
      ? `${summary}.`
      : `Upgrade locked in: ${summary}. It will take effect Sunday at 12:01 AM Eastern.`;
  } catch (error) {
    elements.developmentMessage.textContent = error.message;
    button.disabled = false;
  }
...
function draftSyncSignature(draft = state.draft) {
  return [
    ...
    draft.pool?.length || 0,
    draft.initiation?.retentionRequired ? 1 : 0,
    draft.initiation?.retentionCount || 0,
    draft.initiation?.retentionComplete ? 1 : 0,
    draft.initiation?.coachRequired ? 1 : 0,
    draft.initiation?.coachCount || 0,
    draft.initiation?.coachComplete ? 1 : 0,
    draft.initiation?.pitCoachSelection?.specialtyKey || "",
    draft.initiation?.retentionOptions?.[managedTeamId()]?.selected?.veteranRacerId || "",
    draft.initiation?.retentionOptions?.[managedTeamId()]?.selected?.rookieRacerId || "",
