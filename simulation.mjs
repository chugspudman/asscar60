...
function performanceTimeMultiplier(pace, speed) {
  const combinedRating = 0.7 * clamp(pace, 1, 10) + 0.3 * clamp(speed, 1, 10);
  return linearScale(combinedRating, 2, 1);
}
  ...
  while (racers.some((entry) => entry.status === "running") && time < 7200) {
    ...
    for (const entry of runningAtTick) {
      ...
      while (entry.status === "running" && entry.position >= entry.nextCheckpoint) {
        ...
        entry.lastLapTime = crossingTime - entry.currentPartStart;
        ...
        entry.carWear += degradationOverForty(effectiveDurability)
          / 40
